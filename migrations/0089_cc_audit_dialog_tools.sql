-- 0089 Widen cc_audit_log.tool so observe/confirm dialog operations can
-- land in the append-only ledger. SQLite cannot ALTER a CHECK constraint,
-- so the table is rebuilt. Indexes, triggers, and cc_recent_audit are
-- recreated with the same SQL as 0079.
...
ALTER TABLE cc_audit_log RENAME TO cc_audit_log_0089_old;
...
INSERT INTO cc_audit_log SELECT * FROM cc_audit_log_0089_old;
DROP TABLE cc_audit_log_0089_old;
