-- 0120: allow 'secret.put', 'credential.submitted' and 'audit.export' in the
-- audit action enum (S-07). Secret writes, credential submissions and audit
-- exports are security-relevant mutations that must leave an audit trail on
-- the tamper-evident audit_events chain; without these values an audit insert
-- would abort the enclosing business transaction. SQLite cannot ALTER a CHECK,
-- so the table is rebuilt with the widened enum and re-sealed. The seq /
-- prev_hash / event_hash chain columns are carried forward unchanged so the
-- existing hash chain stays intact, and the two triggers are dropped first and
-- recreated byte-for-byte so the schema-drift guard still matches
-- expectedSchemaSQL.
...
INSERT INTO audit_events (id, action, aggregate_id, actor, metadata_json, created_at, seq, prev_hash, event_hash)
    SELECT id, action, aggregate_id, actor, metadata_json, created_at, seq, prev_hash, event_hash FROM audit_events_old;
