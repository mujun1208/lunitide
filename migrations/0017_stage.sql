...
CREATE INDEX ix_stages_project_phase ON stages(project_id, phase, id);

DROP INDEX ix_idempotency_expires;
ALTER TABLE idempotency_records RENAME TO idempotency_records_0017_old;
...
INSERT INTO idempotency_records SELECT * FROM idempotency_records_0017_old;
DROP TABLE idempotency_records_0017_old;
CREATE INDEX ix_idempotency_expires ON idempotency_records(expires_at);

DROP INDEX ix_audit_aggregate_created;
ALTER TABLE audit_events RENAME TO audit_events_0017_old;
...
INSERT INTO audit_events SELECT * FROM audit_events_0017_old;
DROP TABLE audit_events_0017_old;
