-- 0054: audit_events rebuild — add the 18 M0-M3 storage actions that the
-- catalog missed (review/memory/plan/node/ontology/skill families).
-- Discovered by the 2026-08-15 full-chain audit: governance.go,
-- memory_storage.go, planning.go, ontology.go and skill_storage.go write
-- these actions via execWithAudit, but no migration ever registered them,
-- so every M0-M3 memory/ontology/plan/node/skill/review write failed at
-- runtime with CHECK constraint failed. Rebuild follows the 0047-0053
-- pattern (rename / create / copy / drop / re-index).

DROP INDEX ix_audit_aggregate_created;
ALTER TABLE audit_events RENAME TO audit_events_0054_old;
...
INSERT INTO audit_events SELECT * FROM audit_events_0054_old;
DROP TABLE audit_events_0054_old;
