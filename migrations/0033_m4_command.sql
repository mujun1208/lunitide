-- M4-F: command.start/cancel enter the idempotency surface and
-- command.* audit actions enter the audit trail. Both tables are rebuilt
-- with the extended CHECK lists (same pattern as 0031/0032); no data
-- changes beyond the copy.

-- M4-F: rebuild idempotency_records to admit the command.* operations.
DROP INDEX ix_idempotency_expires;
ALTER TABLE idempotency_records RENAME TO idempotency_records_0033_old;
CREATE TABLE idempotency_records (
    operation TEXT NOT NULL CHECK (operation IN ('provider.create', 'provider.update', 'provider.model.sync', 'provider.delete', 'project.create', 'session.create', 'session.update', 'message.append', 'stage.create', 'message.append-assistant', 'agent.run.start', 'agent.run.resume', 'agent.run.cancel', 'agent.run.reconcile', 'review.decide', 'workspace.register', 'workspace.grant', 'workspace.lease', 'changeset.preview', 'changeset.apply', 'changeset.revert', 'command.start', 'command.cancel')),
...
INSERT INTO idempotency_records SELECT * FROM idempotency_records_0033_old;
DROP TABLE idempotency_records_0033_old;
CREATE INDEX ix_idempotency_expires ON idempotency_records(expires_at);

-- M4-F: rebuild audit_events to admit the command.* audit actions.
DROP INDEX ix_audit_aggregate_created;
ALTER TABLE audit_events RENAME TO audit_events_0033_old;
...
INSERT INTO audit_events SELECT * FROM audit_events_0033_old;
DROP TABLE audit_events_0033_old;
