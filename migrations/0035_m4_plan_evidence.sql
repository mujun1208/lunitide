-- M4-H: run.plan.put enters the idempotency surface and run.plan.updated
-- enters the audit trail. Both tables are rebuilt with the extended CHECK
-- lists (same pattern as 0031/0032/0033/0034); no data changes beyond the
-- copy. evidence.list is a read-only projection and needs neither an
-- idempotency operation nor an audit action.

-- M4-H: rebuild idempotency_records to admit the run.plan.put operation.
DROP INDEX ix_idempotency_expires;
ALTER TABLE idempotency_records RENAME TO idempotency_records_0035_old;
CREATE TABLE idempotency_records (
    operation TEXT NOT NULL CHECK (operation IN ('provider.create', 'provider.update', 'provider.model.sync', 'provider.delete', 'project.create', 'session.create', 'session.update', 'message.append', 'stage.create', 'message.append-assistant', 'agent.run.start', 'agent.run.resume', 'agent.run.cancel', 'agent.run.reconcile', 'review.decide', 'workspace.register', 'workspace.grant', 'workspace.lease', 'changeset.preview', 'changeset.apply', 'changeset.revert', 'command.start', 'command.cancel', 'web.fetch', 'web.search', 'run.plan.put')),
...
INSERT INTO audit_events SELECT * FROM audit_events_0035_old;
DROP TABLE audit_events_0035_old;
