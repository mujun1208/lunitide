-- M6 audit actions (T-6.3.x): delegation lifecycle and barrier arrivals.
-- Rebuild-only migration mirroring 0047's pattern.

DROP INDEX ix_audit_aggregate_created;
ALTER TABLE audit_events RENAME TO audit_events_0048_old;
...
INSERT INTO audit_events SELECT * FROM audit_events_0048_old;
DROP TABLE audit_events_0048_old;
CREATE INDEX ix_audit_aggregate_created ON audit_events(aggregate_id, created_at DESC);

-- Delegation mutations write idempotency records under their own operation
-- names, so the idempotency_records operation CHECK gains delegation.create
-- and delegation.settle (same rebuild pattern, no data changes). barrier
-- arrivals are idempotent through UNIQUE(barrier_id, child_id) and never
-- touch this table.

DROP INDEX ix_idempotency_expires;
ALTER TABLE idempotency_records RENAME TO idempotency_records_0048_old;
...
INSERT INTO idempotency_records SELECT * FROM idempotency_records_0048_old;
DROP TABLE idempotency_records_0048_old;
