-- M4-G: web.fetch/web.search enter the idempotency surface and
-- web.fetched/web.searched enter the audit trail. Both tables are rebuilt
-- with the extended CHECK lists (same pattern as 0031/0032/0033); no data
-- changes beyond the copy.

-- M4-G: rebuild idempotency_records to admit the web.* operations.
DROP INDEX ix_idempotency_expires;
ALTER TABLE idempotency_records RENAME TO idempotency_records_0034_old;
...
INSERT INTO audit_events SELECT * FROM audit_events_0034_old;
DROP TABLE audit_events_0034_old;
