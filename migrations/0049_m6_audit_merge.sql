...
INSERT INTO audit_events SELECT * FROM audit_events_0049_old;
DROP TABLE audit_events_0049_old;
CREATE INDEX ix_audit_aggregate_created ON audit_events(aggregate_id, created_at DESC);

-- merge.submit writes its idempotency record under its own operation name
-- (same rebuild pattern, no data changes). The writer loop and final gate
-- are engine-internal walks: they never re-enter the bridge, so they have
-- no idempotency operation of their own — their durability rides on the
-- intent version CAS and the m6_outbox rows committed in the same
-- transaction.

DROP INDEX ix_idempotency_expires;
ALTER TABLE idempotency_records RENAME TO idempotency_records_0049_old;
...
INSERT INTO idempotency_records SELECT * FROM idempotency_records_0049_old;
DROP TABLE idempotency_records_0049_old;
