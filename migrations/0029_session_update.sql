...
ALTER TABLE idempotency_records RENAME TO idempotency_records_0029_old;
CREATE TABLE idempotency_records (
    operation TEXT NOT NULL CHECK (operation IN ('provider.create', 'provider.update', 'provider.model.sync', 'provider.delete', 'project.create', 'session.create', 'session.update', 'message.append', 'stage.create', 'message.append-assistant')),
...
INSERT INTO idempotency_records SELECT * FROM idempotency_records_0029_old;
DROP TABLE idempotency_records_0029_old;
CREATE INDEX ix_idempotency_expires ON idempotency_records(expires_at);

DROP INDEX ix_audit_aggregate_created;
ALTER TABLE audit_events RENAME TO audit_events_0029_old;
...
INSERT INTO audit_events SELECT * FROM audit_events_0029_old;
DROP TABLE audit_events_0029_old;
