...
Legacy Chromium `v10`/`v11` safeStorage migration used to widen this surface: it unwrapped the Electron AES key with CurrentUser DPAPI and decrypted one AES-GCM value in the Host, and although raw key and plaintext buffers were wiped, Go's `crypto/aes` does not expose destruction of its expanded key schedule. That path was deleted in 0.4.01 along with the prototype it read (see ADR-001), so the Host no longer decrypts anything it did not write.
