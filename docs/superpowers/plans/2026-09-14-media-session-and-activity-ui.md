...
**Spec:** [完整 PRD](../specs/2026-09-14-memory-ocr-media-ui-upgrade-prd.md)，TOOL-001～004、MEDIA-001～010、UI-001～004。
...
- 所有 mutation 顶层 envelope idempotencyKey，payload operationId/expectedRevision；key+subject+method 唯一，重放变参 OPERATION_REPLAY_MISMATCH。
...
| 8 | scripts/test-media-session.ps1；docs/media/session-runbook.md；docs/traceability/media-session.md | 并发、恢复、安全、实机播放与发布证据 |
...
| media_sessions | id/subject/scope/origin、phase/verification、asset_id、position/duration/volume/muted、external app/session key、queue_revision/revision |
...
| media_player_commands | operation_id PK、session/lease generation、absolute desired state、claimed_at/acknowledged_at/state |
...
| media.session.get/list | id 或 scope/cursor/limit → snapshots |
...
| media.asset.list | scope/cursor/limit → authorized assets |
...
| media.player.report | mediaSessionId,leaseToken,generation,operationId,eventSeq,event,positionMs?,durationMs?,errorCode? → receipt |
...
- [ ] **5. Owned 指令闭环。** command 通过治理后写 media_player_commands 的“绝对目标状态”，例如 next 在 Engine CAS 中选定下一 asset，而非给 UI 重放 next。单个 Host window 获30s lease，每10s续约；attach从可信 Host context取得 window identity，不能让 renderer 自选。next 领取后不删除，直到对应 report ack；UI按operationId去重，reconnect只恢复 asset/position到paused，不未经用户意图自动出声。lease争夺/expiry使未完成动作 uncertain。player.report 只允许持有者、匹配generation、单调eventSeq和当前directive target，不接受任意客户端报告 verified flag。
...
- [ ] **6. 自动下一首。** owned ended report匹配当前asset/lease后，Engine依原会话autoAdvance设置（默认true）创建一次linked operation和绝对next directive；重复ended幂等。急停/权限关闭/窗口无lease时停止续播，不能无限重试。
...
数据来自已有 operation.list、OCR pack.get/run.list、media.session.list；适配为只读集合，不增加第四套活动数据库。OCR未启用时可显示空集合，不阻塞media；分页聚合必须按domain cursor保存，不能只读第一页声称完整历史。首版显示最近100项并明确“加载更多”。
- [ ] **4. 卡片。** 目标→等待审批→发送→核验→已确认/无法确认/失败；显示证据来源、elapsed和一个真实recovery。非media仍用ToolTrajectory；SessionPage只挂一套player/store/tray，离开页不重复派发。
- [ ] **5. 权限。** ComputerPanel直接读取现有配置/急停，不复制toggle。关闭电脑控制后external只读；owned本地播放按共用权限定义运行，Agent发起动作仍经过ToolRuntime；不拿桌面锁长期锁住本地音乐。
