...
版本：PRD 1.11 · 2026-09-07（持续追加问题、六类能力与组合链路回归）
代码基线：v0.4.67 · `09978597dc026bfa40ab8d690d64f9bcaef476f3`
状态：**0.4.69 的实机反馈及后续追加需求已重新立项，0.4.70 本轮源码整改和自动回归已完成，未打包，真实环境待用户复测；4.73/5 仅保留为上一轮历史暂评，当前不沿用为验收分数，4.9/5 尚未确认。** 本轮以 [源码交付回执](implementation/feedback-delivery-20260907.md)、[本轮回归整改](implementation/feedback-regressions-20260907.md) 和 [用户复测清单](implementation/user-acceptance-checklist-20260907.md) 为准，既有需求以 [兼容性整改台账](implementation/compatibility-remediation-20260907.md) 为准；历史门禁不能替代本轮组合链路验证。
适用范围：当前 Windows 本地优先桌面产品，不扩张成新的云平台。

本次追加六类能力见 [六类能力落地方案](implementation/six-capability-upgrade-20260907.md)：信息检索、电脑 GUI、浏览器、文档生成、文件系统、自动化与调度。保持同一内核，免费条件逐项明确；无账号永久免费票务/实时行情不作已实现承诺。**用户最新指令为本轮不打包、不安装，交付源码修复、测试证据与统一复测清单**；以下历史发布步骤不得覆盖此指令。

## 1. 决策结论

**历史评定 4.9/5 已撤回作为当前版本的结论。** 用户反馈证明原验收遗漏了真实产品组合：个人容器与正式项目共用创建接口、截图失败后再次发送、系统声音连续取帧、撤销端点重装、引导决定丢失回执后的重试。原 40 项完成状态只覆盖当时的范围，不能覆盖本次新增 C01–C14。当前实施、证据和未验边界见 [兼容性整改台账](implementation/compatibility-remediation-20260907.md) 及 [本次交付复核](implementation/compatibility-delivery-20260907.md)。

历史六组门禁及其源码摘要 `e578444c4485345e8aa55dcc2b71e551e2abda0d07627e23cc149e62d50fef17` 保存在 [第二阶段验证报告](implementation/phase2-verification.md)。它们不能证明当前源码或 0.4.69 候选通过。真实设备、服务账户、签名安装和长期观察继续由用户验收，代码缺口继续由实施方处理。
...
上一轮 0.4.69 的普通 Go 为 4,859 项通过，前端全量 1,705 项通过，随后完整语音目录 553 项通过。竞态采用全库运行加修正/分组复测去重核对，4,853 项通过、28 项跳过；原全库退出 1、SQLite 90 分钟超时及 6 个桌面 `!race` 排除项均明确保留，不能概括为所有门禁一次全绿。候选 R2 已独立核验；完整结果见 [本次交付复核](implementation/compatibility-delivery-20260907.md)。
...
| 验收对象 | 责任 | 完成条件 |
|---|---|---|
| 代码升级交付 | 实施方 | 原40项与本轮C01–C14台账中适用的实现关闭；反例有正式正确性回归；全量Go/前端、全库race、迁移、契约、静态、类型、构建、依赖扫描与发布工具门禁通过；源码摘要和日志可复查。 |
