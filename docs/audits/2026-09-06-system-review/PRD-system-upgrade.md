...
版本：PRD 1.8 · 2026-09-07（兼容性修复、候选核验与证据对应）
代码基线：v0.4.67 · `09978597dc026bfa40ab8d690d64f9bcaef476f3`
状态：**已落实本轮 C01–C14 兼容性修复和交互补充，修正版 R2 候选已独立核验；自动化复核及竞态失败/超时补测已收尾，真实环境验证独立待验，当前工程暂评 4.73/5，尚未确认 4.9/5。** 当前需求和进度以 [兼容性整改台账](implementation/compatibility-remediation-20260907.md) 为准。下文第二阶段评分及原日志保留为历史证据，不代表当前版本已经验收。
...
当前最终普通 Go 为 4,859 项通过，前端全量 1,705 项通过，随后完整语音目录 553 项通过。竞态采用全库运行加修正/分组复测去重核对，4,853 项通过、28 项跳过；原全库退出 1、SQLite 90 分钟超时及 6 个桌面 `!race` 排除项均明确保留，不能概括为所有门禁一次全绿。候选 R2 已独立核验；完整结果见 [本次交付复核](implementation/compatibility-delivery-20260907.md)。
