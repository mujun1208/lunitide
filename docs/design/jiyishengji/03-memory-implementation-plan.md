...
- 除合同声明的lease/report/claim内部幂等协议外，所有业务mutation 的 idempotencyKey 在既有 Bridge envelope 顶层，operationId/expectedRevision 在 payload；服务端计算 request digest。
...
| 9 | testdata/memory-fabric-v2/*.jsonl；scripts/eval-memory-fabric-v2.ps1；docs/design/jiyishengji/evidence/memory-fabric-v2.md | 固定评测与发布证据 |
...
capture job只存source引用、游标、lease_owner/lease_until/heartbeat_at/fence，UNIQUE(subject_id,source_message_id,source_revision)。embedding job按fact/version/embedding_space_id唯一。v2_settings整数revision、五flags默认0；现有memory.settings的SHA合同保持兼容。

- [ ] **5. 接迁移并绿测。** 更新store.go的真实manifest/expectedSchemaSQL和列清单。memory_search_documents保存rowid、subject_id、scope_kind、scope_id、fact_id、fact_version、text；同fact/version唯一，rowid作为memory_search_fts的external-content rowid。INSERT/UPDATE/DELETE triggers维护FTS；所有检索join完整授权键及head/历史有效区间，forget先删shadow触发索引删除，rebuild从未忘记body重建。
...
- [ ] **3. 实现 durable scheduling。** 回合事务内或通过带 checkpoint 的启动补扫，保证已持久化 user turn 都可找到 capture job；内存 channel 只唤醒。claim 租约 60s、每 15s heartbeat；进程重启回收过期 lease。队列达到10000时停止物化新普通job并保留source cursor，继续claim消费以排空，不能丢弃用户纠正/删除；secret/明确负例仅保留原因计数。重试 1/5/30/120/600s，五次后 deferred，模型配置恢复可重新入队。
...
- [ ] **6. 计费接线。** 写 memory_model_usage（purpose=extract|consolidate|embed|query_embed），同时在现有 call-purpose wrapper 标注 memory.extract 等。reported 缺失写 NULL，不写 0 假装免费；估算与真实账单分开。
...
**新增公开 methods：** memory.item.list/get/history/correct/forget、memory.capture.undo、memory.review.list/resolve、memory.purge.prepare。保留并扩展 memory.facts.flag、memory.settings.get/update；不另造 hide/pin API。
...
| item.get | factId,version?,asOf?（version/asOf互斥） → authorized item/head/source metadata |
| item.history | factId,cursor?,limit(1..100) → version metadata,nextCursor |
| item.correct | factId,replacementText,validFrom?,reason,operationId,expectedRevision → new item |
| item.forget | factId,targetVersion?,mode=this_version\|fact_history\|subject_rule,ruleCategory?,operationId,expectedRevision → scrub receipt |
| capture.undo | undoOperationId,operationId → 原批次撤销receipt；任一项后续修改则整批MEMORY_UNDO_CONFLICT |
...
- [ ] **5. 授权 UI。** settings 扩展 capture policies/modelAssist，旧 schema 显式增字段后生成；新增公开请求不传subject；旧memory.settings保留subjectId字段并与Engine身份比较，拒绝不匹配。异常映射稳定错误码，所有正文安全文本渲染。键盘、焦点返回、200% zoom、读屏 status 验证。
...
- [ ] **6. 提交/发布。** 记录 requirements→test→commit→artifact 映射到 docs/design/jiyishengji/evidence/memory-fabric-v2.md，提交 `test(memory): record reproducible fabric release gates`。数据迁移/基础 read gate 通过后才启用 auto，不再要求用户逐条选择记忆。

## 依赖顺序与完成定义

X0备份/基线 → Task1 → Task2 → Task4/7数据保护 → Task3 → Task5 → Task6 → Task8 → X1/X2/X3集成 → Task9。corpus/旧基线从 Task1 前开始，避免事后调指标。
