...
// 快到期的待审批项（≤30 分钟）计为“即将过期”，对应原型 meta「1 项即将过期」
const expiringSoon = (reviews: ReviewDTO[]): number => reviews.filter(r => r.status === 'pending' && r.expiresAt && new Date(r.expiresAt).getTime() - Date.now() > 0 && new Date(r.expiresAt).getTime() - Date.now() < 30 * 60000).length
...
export function ReviewPage({ projectId, bridge = reviewBridge, plans = planBridge }: { projectId: string; bridge?: ReviewBridge; plans?: PlanBridge }): React.JSX.Element {
  ...
  const meta = selectedPlanId
    ? `${planName(selectedPlanId)} · 待我审批 ${pending.length}${expiringSoon(pending) ? ` · ${expiringSoon(pending)} 项即将过期` : ''}`
    : '请先选择计划'

  return (
    <div className="approval-center">
      <header className="expert-view-head">
        <div><div className="view-title">审批中心</div><div className="view-meta">{meta}</div></div>
        <div className="memory-tabs" style={{ marginLeft: 'auto' }} role="tablist" aria-label="审批分区">
      ...
      {error && <div className="error" role="alert"><b>{error}</b>{retryable && <button onClick={() => void load(selectedPlanId)}>重试</button>}</div>}
      {(tab === 'pending' || tab === 'done') && (<>
        <div className="memory-toolbar">
          <label className="appr-plan">计划
        ...
        {!selectedPlanId ? (
          <div className="empty"><b>请选择计划</b><span>审批记录按计划归集；从下拉列表选择一个计划查看。</span></div>
        ) : loading ? <p role="status">正在载入审批…</p>
        : shown.length === 0 ? (
          <div className="empty"><b>{tab === 'pending' ? '暂无待审批请求' : '暂无已处理记录'}</b><span>{tab === 'pending' ? '新的高风险操作会在这里等待你的批准。' : '处理过的审批会归档在这里。'}</span></div>
        ) : (
          <div className="appr-list">
            {shown.map(review => (
              <div key={review.id} className={`appr-item ${riskClass(review.riskLevel)}`}>
                <div className="appr-head">
                  <span className="a-type">{review.actionType}</span>
                  <span className={`a-risk ${riskClass(review.riskLevel)}`}>{RISK_LABELS[review.riskLevel]}</span>
                  <span className="mono a-id">{review.id.slice(0, 8).toUpperCase()}</span>
                  {review.status === 'pending' && review.expiresAt && <span className="a-expire">{expireText(review.expiresAt)}</span>}
                  {review.status !== 'pending' && <span className="a-expire">{REVIEW_STATUS_LABELS[review.status]}{review.reviewedAt ? ` · ${new Date(review.reviewedAt).toLocaleString()}` : ''}</span>}
                </div>
                <div className="appr-body">
                  操作类型: <span className="kv">{review.actionType}</span><br />
                  参数摘要: action <span className="kv">{review.actionDigest.slice(0, 8)}…{review.actionDigest.slice(-4)}</span> · input <span className="kv">{review.inputDigest.slice(0, 8)}…{review.inputDigest.slice(-4)}</span> · state <span className="kv">{review.stateDigest.slice(0, 8)}…{review.stateDigest.slice(-4)}</span><br />
                  计划版本: <span className="kv">{planName(review.planId)}</span> · 策略版本: <span className="kv">policy v{review.policyVersion}</span>{review.nodeId ? <> · 节点: <span className="kv">{review.nodeId.slice(0, 8)}</span></> : ''}
                </div>
                {review.reviewerNote && <div className="appr-body"><b>审批备注</b>：{review.reviewerNote}</div>}
                {review.status === 'pending' && (
                  <div className="appr-actions">
                    <input value={notes[review.id] ?? ''} onChange={e => setNotes(prev => ({ ...prev, [review.id]: e.target.value }))} placeholder="审批备注（可选）" aria-label={`审批备注 ${review.id}`} />
                    <button className="ui-btn danger" disabled={busy} onClick={() => void doReview('reject', review.id)}>退回</button>
                    <button className="ui-btn primary" disabled={busy} onClick={() => void doReview('approve', review.id)}>{busy ? '处理中…' : '批准此操作'}</button>
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="callout"><b>审批原则</b>：超时默认暂停；操作参数发生变化后原批准自动失效（变更后失效状态）。批准不绕过执行前重验；摘要、环境或策略漂移时批准立即失效。</div>
      </>)}
      {tab === 'auto' && (
        <>
