...
import { CAPABILITY_PACKS, PACK_LEDGER_KEY } from './capabilityPacks'
...
it('lists a chat-created pack from plugin.list without localStorage', async () => {
  ...
  expect(screen.getByText('历史卡片，依赖待核验')).toBeInTheDocument()
})

it('removes a capability pack using the persisted version and keeps skills',async()=>{
 const record={spec:CAPABILITY_PACKS[0],digest:'a'.repeat(64),state:'installed',desired:'installed',version:4,error:'',createdAt:now,updatedAt:now,components:[]}
 const bridge=api({packList:vi.fn().mockResolvedValue({items:[record]})})
