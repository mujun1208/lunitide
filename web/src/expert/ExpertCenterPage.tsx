...
const mcpEndpointMatchesPreset=(ep:McpListResult['endpoints'][number],id:string)=>{
 if(ep.state==='revoked'||ep.state==='quarantined')return false
 const hay=`${ep.displayName??''} ${ep.command??''} ${(ep.args??[]).join(' ')}`.toLowerCase()
...
export function ExpertCenterPage({bridge=expertBridge,projects=projectBridge,skills=defaultSkillBridge,mcp,onCreateInChat,onOpenExpert,onUseInSession,onOpenWorkbench,initialSelectedId}:{bridge?:ExpertBridge;projects?:ProjectBridge;skills?:SkillBridge;mcp?:McpBridge;onCreateInChat?:()=>void;onOpenExpert?:(item:ExpertItem)=>void;onUseInSession?:(item:ExpertItem)=>void;onOpenWorkbench?:(item:ExpertItem)=>void;initialSelectedId?:string}):React.JSX.Element{
 ...
 const[detailWidth,startDetailResize]=usePanelResize({storageKey:'lunitide:expert-detail-width',initial:380,min:280,max:()=>Math.min(Math.round(window.innerWidth*0.85),Math.max(420,window.innerWidth-280)),reverse:true})
 const zh=useZh()

 const load=useCallback(async(preferredId?:string)=>{setLoading(true);setError('');try{const result=await bridge.list({});setItems(result.experts);setSelectedId(current=>{if(preferredId&&result.experts.some(item=>item.expertId===preferredId))return preferredId;if(!current&&initialSelectedId&&result.experts.some(item=>item.expertId===initialSelectedId))return initialSelectedId;return result.experts.some(item=>item.expertId===current)?current:(result.experts[0]?.expertId??'')})}catch(e){setError(expertUserError(e,'专家清单加载失败'))}finally{setLoading(false)}},[bridge,initialSelectedId])
 const loadCatalog=useCallback(async()=>{if(!bridge.catalogList){setMarketError('当前版本未提供专家目录。');return}setMarketLoading(true);setMarketError('');try{const result=await bridge.catalogList({});setCatalog(result.items??[])}catch(e){setCatalog([]);setMarketError(e instanceof BridgeClientError?formatBridgeFailure(e,'专家市场加载失败'):expertUserError(e,'专家市场加载失败'))}finally{setMarketLoading(false)}},[bridge])
 ...
 const visible=useMemo(()=>items.filter(item=>(!manualOnly||(item.creationOrigin==='manual'&&item.isOwn))&&(!stateFilter||item.state===stateFilter)&&(!kindFilter||expertKindOf(item)===kindFilter)&&(!divisionFilter||item.division===divisionFilter)&&(!query.trim()||displayName(item).toLowerCase().includes(query.trim().toLowerCase()))),[divisionFilter,items,kindFilter,manualOnly,query,stateFilter])
 const selected=visible.find(item=>item.expertId===selectedId)??visible[0]
 useEffect(()=>{const expertId=selected?.expertId;if(!expertId){setDetail(null);setScenarios([]);setScenarioForm(EMPTY_SCENARIO);setSkillDraft([]);setMcpDraft([]);setBrainDraft('lunitide');return}let alive=true;setDetail(null);setScenarios([]);setScenarioForm(EMPTY_SCENARIO);Promise.all([bridge.detail({expertId}),bridge.scenarioList({expertId,state:'active'})]).then(([next,cards])=>{if(!alive)return;setDetail(next);setScenarios(cards.items);const name=String(asRecord(next.expert).name??'');const stored=splitBoundKeys(boundSkillKeys(next.expert));const known=boundSkillsKnown(next.expert);setSkillDraft(stored.skills.length||known?stored.skills:preferredSkillsForExperts([{name}]));setMcpDraft(stored.mcp.length||known?stored.mcp:preferredMcpForExperts([{name}]));setBrainDraft(stored.brain)}).catch(e=>{if(!alive)return;setDetail(null);setScenarios([]);setError(expertUserError(e,'专家详情加载失败'))});return()=>{alive=false}},[bridge,detailEpoch,selected?.expertId])
 useEffect(()=>{if(!bridge.skillsSet)return;let alive=true;void skills.list({status:'published'}).then(result=>{if(alive){setPublishedSkills(result.items.filter(item=>item.status==='published'));setSkillsReady(true)}}).catch(()=>{if(alive){setPublishedSkills([]);setSkillsReady(true)}});return()=>{alive=false}},[bridge.skillsSet,skills])
 useEffect(()=>{let alive=true;const client=mcp??(()=>{try{return getMcpBridge()}catch{return undefined}})();if(!client?.presets)return;void client.presets().then(result=>{if(alive)setMcpPresets((result.items??[]).filter(item=>leftoverArchivedMcp(item.args,item.url).length===0).map(item=>({id:item.id,name:item.name,description:item.description})))}).catch(()=>{if(alive)setMcpPresets([])});return()=>{alive=false}},[mcp])
 useEffect(()=>{let alive=true;const client=mcp??(()=>{try{return getMcpBridge()}catch{return undefined}})();if(!client?.list){setMcpReady(true);return};void client.list({}).then(result=>{if(alive){setMcpEndpoints((result.endpoints??[]).filter(item=>item.state!=='revoked'&&leftoverArchivedMcp(item.args,item.url).length===0));setMcpReady(true)}}).catch(()=>{if(alive){setMcpEndpoints([]);setMcpReady(true)}});return()=>{alive=false}},[mcp])

 const[editBase,setEditBase]=useState<{expertId:string;versionId:string}|null>(null)
 const beginCreate=()=>{setAddOpen(false);setEditing('create');setError('');setForm(EMPTY_FORM)}
 const beginEdit=()=>{if(!selected||!detail)return;const versionId=expertMeta(detail.expert).currentVersionId;if(!versionId){setError('当前版本不可用，无法保存');return}setEditBase({expertId:selected.expertId,versionId});const body=sixOf(detail.sixSection);setEditing('edit');setError('');setForm({...EMPTY_FORM,...body,name:displayName(selected),division:selected.division,semver:selected.semver,changeNote:''})}
 ...
 const submit=async()=>{if(editing==='create'){const problem=validateCreate();if(problem){setError(problem);return}setBusy(true);setError('');setNotice('');try{const base:ExpertCreatePayload={source:'local',frontmatter:{name:form.name.trim(),division:form.division,description:form.description.trim(),semver:form.semver.trim()},sixSection:sixPayload(),requestId:crypto.randomUUID()},attempt=createMutationAttempt('expert.create',base);const created=await bridge.create(attempt.payload,{attempt});setNotice(`专家「${created.name||form.name.trim()}」已创建，尚未启用`);setEditing(null);setManualOnly(true);setStateFilter('');setKindFilter('');setDivisionFilter('');setQuery('');await load(created.expertId);setDetailEpoch(value=>value+1)}catch(e){setError(expertUserError(e,'专家保存失败'))}finally{setBusy(false)};return}
  if(editing!=='edit'||!selected||!editBase)return;const problem=validateSix();if(problem){setError(problem);return}if(form.changeNote.trim().length<1||form.changeNote.length>2000){setError('变更说明需为 1–2000 个字符');return}
  const expectedVersionId=editBase.versionId;if(!expectedVersionId){setError('当前版本不可用，无法保存');return}
  setBusy(true);setError('');setNotice('');try{const base={expertId:editBase.expertId,expectedVersionId,sixSection:sixPayload(),changeNote:form.changeNote.trim()},attempt=createMutationAttempt('expert.update',base);const result=await bridge.update(base,{attempt});setNotice(`专家「${displayName(selected)}」已更新为 ${result.semver}`);setEditing(null);await refresh()}catch(e){setError(expertUserError(e,'专家保存失败'))}finally{setBusy(false)}}
 const toggle=async(item:ExpertItem)=>{setBusy(true);setError('');try{const base={expertId:item.expertId,enabled:item.state!=='enabled'},attempt=createMutationAttempt('expert.toggle',base);const result=await bridge.toggle(base,{attempt});setNotice(`专家「${displayName(item)}」已${result.state==='enabled'?'启用':'停用'}`);await refresh()}catch(e){setError(expertUserError(e,'专家状态更新失败'))}finally{setBusy(false)}}
 const archive=async()=>{if(!archiving)return;setBusy(true);setError('');setNotice('');try{const confirmToken=await sha256Hex(`expert.archive|${archiving.expertId}`),base={expertId:archiving.expertId,confirmToken},attempt=createMutationAttempt('expert.archive',base);await bridge.archive(base,{attempt});setNotice(`专家「${displayName(archiving)}」已归档`);setArchiving(null);await refresh()}catch(e){setError(expertUserError(e,'专家归档失败'))}finally{setBusy(false)}}
 ...
 const presentSkillFloor=skillFloor.filter(key=>publishedSkills.some(item=>skillMatchesPreferred(item,[key])))
 const presentMcpFloor=mcpFloor.filter(id=>installedMcpIds.has(id))
 useEffect(()=>{if(!skillsReady)return;setSkillDraft(current=>{const known=new Set(publishedSkills.flatMap(item=>[item.name,item.name.replace(/^tpl-/,''),item.entryPoint?.replace(/^builtin:\/\//,'')??'']));const kept=current.filter(key=>known.has(key));const next=[...new Set([...kept,...presentSkillFloor])];return next.length===current.length&&next.every((key,i)=>key===current[i])?current:next})},[presentSkillFloor,publishedSkills,skillsReady])
 useEffect(()=>{if(!mcpReady)return;setMcpDraft(current=>{const kept=current.filter(id=>installedMcpIds.has(id));const next=[...new Set([...kept,...presentMcpFloor])];return next.length===current.length&&next.every((id,i)=>id===current[i])?current:next})},[installedMcpIds,mcpReady,presentMcpFloor])
 ...
 const saveSkills=async()=>{if(!selected||!detail||!bridge.skillsSet)return;const expectedVersionId=expertMeta(detail.expert).currentVersionId;if(!expectedVersionId){setError('当前版本不可用，无法保存装备');return};setBusy(true);setError('');setNotice('');try{const merged=[...new Set([...presentSkillFloor,...skillDraft,...[...new Set([...presentMcpFloor,...mcpDraft])].map(mcpBindKey),...(brainDraft!=='lunitide'?[brainBindKey(brainDraft)]:[])])];const base={expertId:selected.expertId,expectedVersionId,skillKeys:merged},attempt=createMutationAttempt('expert.skills.set',base);const result=await bridge.skillsSet(base,{attempt});const split=splitBoundKeys(result.skillKeys??merged);setSkillDraft(split.skills);setMcpDraft(split.mcp);setBrainDraft(split.brain);setNotice(`已更新「${displayName(selected)}」的运行时绑定`);setDetailEpoch(value=>value+1)}catch(e){setError(expertUserError(e,'运行时绑定失败'))}finally{setBusy(false)}}
 ...
 return <main className="skill-center expert-center-page">
  ...
  <section className="skill-center-toolbar">
   ...
   {view==='library'&&<>
    <div className="skill-status-tabs" role="tablist" aria-label="专家来源"><button type="button" role="tab" aria-selected={!manualOnly} onClick={()=>setManualOnly(false)}>全部来源</button><button type="button" role="tab" aria-selected={manualOnly} onClick={()=>setManualOnly(true)}>我创建的</button></div>
  ...
  :<div className="skill-center-layout" style={{'--detail-width':`${detailWidth}px`} as React.CSSProperties}>
   <section className="skill-table expert-list" aria-label="已安装专家"><div className="skill-table-inner"><div className="skill-table-head"><span>专家</span><span>版本</span><span>状态</span><span>条线</span></div>{loading?<p role="status">正在载入专家…</p>:visible.length?visible.map(item=>{const key=expertCatalogKey(item);const matchedSkillCount=preferredSkillsForExperts([{name:displayName(item),id:key}]).filter(k=>publishedSkills.some(s=>skillMatchesPreferred(s,[k]))).length;const matchedMcpCount=preferredMcpForExperts([{name:displayName(item),id:key}]).filter(id=>installedMcpIds.has(id)).length;return <button type="button" className={`skill-row ${selected?.expertId===item.expertId?'active':''}`} key={item.expertId} onClick={()=>{setSelectedId(item.expertId);setEditing(null)}}><span><b>{displayName(item)}</b><small>{kindLabel(expertKindOf(item))} · 技能 {matchedSkillCount} · MCP {matchedMcpCount} · 挂载 {item.mountedPhaseCount} 处 · {item.creationOrigin==='manual'&&item.isOwn?'我创建的':SOURCE[item.source]??item.source}</small></span><code>v{item.semver||'—'}</code><i className={`skill-status ${STATUS_CLASS[item.state]}`}>{STATES[item.state]??item.state}</i><code>{displayDivision(item)}</code></button>}):<div className="empty"><b>暂无专家</b><span>{query||stateFilter||kindFilter||divisionFilter||manualOnly?'没有匹配的专家。':showMarket?'去「专家市场」安装，或点击「添加专家」用对话生成。':'点击「添加专家」用对话生成。'}</span></div>}</div></section>
   ...
   <aside className="skill-detail expert-detail" aria-label="专家详情">{selected?<>
    ...
    <div className="skill-detail-actions">
     <ExpertLifecycleActions key={selected.expertId} item={selected} versionId={detailReady&&detail?expertMeta(detail.expert).currentVersionId:''} bridge={bridge} disabled={busy} onDeleted={async()=>{setNotice(`专家「${displayName(selected)}」已删除`);await refresh()}}/>
     ...
     {selected.state!=='archived'&&selected.source!=='builtin'&&selected.creationOrigin!=='builtin'&&selected.creationOrigin!=='manual'?<button type="button" className="danger" disabled={busy} onClick={()=>setArchiving(selected)}>归档</button>:null}
    </div>
    <ExpertDetailTabs
     overview={<>
    <h3>运行时</h3>
    ...
    {selected.state!=='archived'&&bridge.skillsSet?<form className="scenario-create" onSubmit={e=>{e.preventDefault();void saveSkills()}}>
     ...
     <div className="dialog-actions"><button type="button" disabled={busy||!detailReady} onClick={()=>{const hasFloor=presentSkillFloor.length>0||presentMcpFloor.length>0;if(hasFloor){const nextSkills=[...new Set([...skillDraft,...presentSkillFloor])];const nextMcp=[...new Set([...mcpDraft,...presentMcpFloor])];setSkillDraft(nextSkills);setMcpDraft(nextMcp);setNotice('已按岗位底线补齐技能与 MCP，正在保存…');setTimeout(()=>void saveSkills(),80)}else{const allSkillKeys=publishedSkills.map(item=>item.entryPoint?.replace(/^builtin:\/\//,'')||item.name.replace(/^tpl-/,''));const allMcpIds=[...installedMcpIds];setSkillDraft(allSkillKeys);setMcpDraft(allMcpIds);setNotice('已勾选全部已安装的技能与 MCP，正在保存…');setTimeout(()=>void saveSkills(),80)}}}>自动匹配并保存</button><button type="button" className="primary" disabled={busy||!detailReady} onClick={()=>void saveSkills()}>保存运行时绑定</button></div>
    </form>:null}
    <h3>六段说明书</h3>
    ...
    {selected.state!=='archived'&&<form className="scenario-create" onSubmit={e=>{e.preventDefault();void submitScenario()}}>
     ...
     knowledge={selected?<ExpertKnowledgePanel expertId={selected.expertId} knowledgeGet={bridge.knowledgeGet} knowledgeIngest={bridge.knowledgeIngest} knowledgeDelete={bridge.knowledgeDelete}/>:null}
