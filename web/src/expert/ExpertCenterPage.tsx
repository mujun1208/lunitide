...
import{createMutationAttempt,expertBridge,projectBridge,skillBridge as defaultSkillBridge,type ExpertBridge,type ProjectBridge,type SkillBridge}from'../bridge/client'
...
import{conversationExpertEmoji,conversationExpertKind,conversationExpertRole,preferredSkillsForExperts}from'./conversationExperts'
...
const expertKindOf=(item:Pick<ExpertItem,'name'> & {kind?:string})=>item.kind==='agent'||item.kind==='prompt_skill'?item.kind:conversationExpertKind(item.name)
const kindLabel=(kind:'agent'|'prompt_skill')=>kind==='agent'?'独立智能体':'技能包'
...
const catalogUsage=(entry:CatalogEntry)=>((entry as {kind?:string}).kind??conversationExpertKind(entry.id))==='agent'?'独立智能体':USAGE[entry.usage]
...
export function ExpertCenterPage({bridge=expertBridge,projects=projectBridge,skills=defaultSkillBridge,onCreateInChat,onOpenExpert}:{bridge?:ExpertBridge;projects?:ProjectBridge;skills?:SkillBridge;onCreateInChat?:()=>void;onOpenExpert?:(item:ExpertItem)=>void}):React.JSX.Element{
 ...
 const[publishedSkills,setPublishedSkills]=useState<SkillDTO[]>([]),[skillDraft,setSkillDraft]=useState<string[]>([])
 ...
 useEffect(()=>{const expertId=selected?.expertId;if(!expertId){setDetail(null);setScenarios([]);setScenarioForm(EMPTY_SCENARIO);setSkillDraft([]);return}let alive=true;setDetail(null);setScenarios([]);setScenarioForm(EMPTY_SCENARIO);Promise.all([bridge.detail({expertId}),bridge.scenarioList({expertId,state:'active'})]).then(([next,cards])=>{if(!alive)return;setDetail(next);setScenarios(cards.items);const stored=boundSkillKeys(next.expert);setSkillDraft(stored.length?stored:(asRecord(next.expert).boundSkills?[]:preferredSkillsForExperts([{name:String(asRecord(next.expert).name??'')}])))}).catch(e=>{if(!alive)return;setDetail(null);setScenarios([]);setError(e instanceof Error?e.message:'专家详情加载失败')});return()=>{alive=false}},[bridge,detailEpoch,selected?.expertId])
 ...
 const skillFloor=selected&&expertKindOf(selected)==='agent'?preferredSkillsForExperts([{name:displayName(selected)}]):[]
 const saveSkills=async()=>{if(!selected||!bridge.skillsSet)return;setBusy(true);setError('');setNotice('');try{const merged=[...new Set([...skillFloor,...skillDraft])];const base={expertId:selected.expertId,skillKeys:merged},attempt=createMutationAttempt('expert.skills.set',base);const result=await bridge.skillsSet(base,{attempt});setSkillDraft(result.skillKeys??merged);setNotice(`已更新「${displayName(selected)}」的挂载技能`);setDetailEpoch(value=>value+1)}catch(e){setError(e instanceof Error?e.message:'技能挂载失败')}finally{setBusy(false)}}
  ...
  :<div className="skill-center-layout" style={{'--detail-width':`${detailWidth}px`} as React.CSSProperties}>
   ...
   <aside className="skill-detail expert-detail" aria-label="专家详情">{selected?<>
    ...
    <div className="skill-detail-actions">
     {selected.state==='enabled'&&onOpenExpert?<button type="button" disabled={busy} onClick={()=>onOpenExpert(selected)}>打开专家</button>:null}
    ...
    <h3>挂载技能</h3>
    <p>技能挂在这位专家身上，对话输入框不会自动出现技能芯片。</p>
