...
export function ExpertCenterPage({bridge=expertBridge,projects=projectBridge,skills=defaultSkillBridge,mcp,onCreateInChat,onOpenExpert,onUseInSession,onOpenWorkbench,initialSelectedId}:{bridge?:ExpertBridge;projects?:ProjectBridge;skills?:SkillBridge;mcp?:McpBridge;onCreateInChat?:()=>void;onOpenExpert?:(item:ExpertItem)=>void;onUseInSession?:(item:ExpertItem)=>void;onOpenWorkbench?:(item:ExpertItem)=>void;initialSelectedId?:string}):React.JSX.Element{
 ...
 useEffect(()=>{const expertId=selected?.expertId;if(!expertId){setDetail(null);setScenarios([]);setScenarioForm(EMPTY_SCENARIO);setSkillDraft([]);setMcpDraft([]);setBrainDraft('lunitide');return}let alive=true;setDetail(null);setScenarios([]);setScenarioForm(EMPTY_SCENARIO);Promise.all([bridge.detail({expertId}),bridge.scenarioList({expertId,state:'active'})]).then(([next,cards])=>{if(!alive)return;setDetail(next);setScenarios(cards.items);const name=String(asRecord(next.expert).name??'');const stored=splitBoundKeys(boundSkillKeys(next.expert));setSkillDraft(stored.skills.length?stored.skills:preferredSkillsForExperts([{name}]));setMcpDraft(stored.mcp.length?stored.mcp:preferredMcpForExperts([{name}]));setBrainDraft(stored.brain)}).catch(e=>{if(!alive)return;setDetail(null);setScenarios([]);setError(expertUserError(e,'专家详情加载失败'))});return()=>{alive=false}},[bridge,detailEpoch,selected?.expertId])
 ...
 const saveSkills=async()=>{if(!selected||!detail||!bridge.skillsSet)return;const expectedVersionId=expertMeta(detail.expert).currentVersionId;if(!expectedVersionId){setError('当前版本不可用，无法保存装备');return};setBusy(true);setError('');setNotice('');try{const merged=[...new Set([...skillFloor,...skillDraft,...[...new Set([...mcpFloor,...mcpDraft])].map(mcpBindKey),...(brainDraft!=='lunitide'?[brainBindKey(brainDraft)]:[])])];const base={expertId:selected.expertId,expectedVersionId,skillKeys:merged},attempt=createMutationAttempt('expert.skills.set',base);const result=await bridge.skillsSet(base,{attempt});const split=splitBoundKeys(result.skillKeys??merged);setSkillDraft(split.skills);setMcpDraft(split.mcp);setBrainDraft(split.brain);setNotice(`已更新「${displayName(selected)}」的运行时绑定`);setDetailEpoch(value=>value+1)}catch(e){setError(expertUserError(e,'运行时绑定失败'))}finally{setBusy(false)}}
