 ...
 return <main className="skill-center expert-center-page">
  ...
  :<div className="skill-center-layout" style={{'--detail-width':`${detailWidth}px`} as React.CSSProperties}>
   ...
   <aside className="skill-detail expert-detail" aria-label="专家详情">{selected?<>
    ...
    {selected.state!=='archived'&&expertKindOf(selected)==='agent'?<fieldset className="expert-mount-steps" aria-label="大脑"><legend>大脑</legend><label className="expert-mount-step"><input type="radio" name="expert-brain" checked={brainDraft==='lunitide'} onChange={()=>setBrainDraft('lunitide')}/><span><b>月汐引擎</b></span></label><label className="expert-mount-step"><input type="radio" name="expert-brain" checked={brainDraft==='codex'} onChange={()=>setBrainDraft('codex')}/><span><b>本机 Codex</b><small> 一次性执行，可接上上一轮；失败会写「已改用月汐引擎」，密钥留在已登录的 CLI</small></span></label><label className="expert-mount-step"><input type="radio" name="expert-brain" checked={brainDraft==='claude'} onChange={()=>setBrainDraft('claude')}/><span><b>本机 Claude Code</b><small> 一次性执行，可接上上一轮；失败会写「已改用月汐引擎」，密钥留在已登录的 CLI</small></span></label></fieldset>:null}
