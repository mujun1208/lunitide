...
function Harness({reverse=false}:{reverse?:boolean}){
 const[size,start]=usePanelResize({storageKey:'panel-test',initial:300,min:200,max:()=>500,reverse})
...
describe('usePanelResize',()=>{
 ...
 afterEach(cleanup)
 const pointer=(target:EventTarget,type:string,clientX:number,button=0)=>act(()=>{const event=new Event(type,{bubbles:true});Object.defineProperties(event,{clientX:{value:clientX},button:{value:button}});target.dispatchEvent(event)})
