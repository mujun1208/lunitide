...
type ResizeOptions={storageKey:string;initial:number;min:number;max:()=>number;reverse?:boolean}

export function usePanelResize({storageKey,initial,min,max,reverse=false}:ResizeOptions):[number,(event:ReactPointerEvent<HTMLDivElement>)=>void]{
 ...
 const start=useCallback((event:ReactPointerEvent<HTMLDivElement>)=>{
  ...
  const originX=event.clientX,originSize=size,body=document.body,previousCursor=body.style.cursor,previousSelection=body.style.userSelect
  body.style.cursor='col-resize';body.style.userSelect='none'
  const move=(next:PointerEvent)=>{const delta=(next.clientX-originX)*(reverse?-1:1);setSize(clamp(originSize+delta))}
