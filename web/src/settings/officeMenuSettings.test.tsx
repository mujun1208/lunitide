...
it('does not keep AgentHub as an office-menu toggle and ignores leftover stored agentHub', () => {
  ...
  expect(loadOfficeMenu()).toEqual({ people: false, mro: false, office: true, meetings: false, media: true, automation: true, productHub: true })
...
it('lets Media Center and Automation be hidden without changing other keys', () => {
  ...
  expect(loadOfficeMenu()).toEqual({ people: false, mro: false, office: true, meetings: false, media: false, automation: false, productHub: true })
})
...
it('shares changes between mounted consumers and retains them after remount without changing other keys', () => {
  ...
  expect(screen.getByRole('switch', { name: '产品总览' })).toHaveAttribute('aria-checked', 'true')
