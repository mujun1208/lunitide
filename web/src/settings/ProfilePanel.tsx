...
function profileUserError(err: unknown, fallback: string): string {
...
export function ProfilePanel({
  ...
  const load = async () => {
    try {
      ...
      setNotice(profileUserError(e, '无法读取个人资料'))
    }
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      setNotice(profileUserError(e, '保存失败'))
    } finally {
      setBusy(false)
    }
  }
  ...
  const setStatus = async (status: IdentityDTO['status']) => {
    ...
    try {
      ...
      setNotice(profileUserError(e, '状态更新失败'))
    } finally {
      setBusy(false)
    }
  }
  ...
  const pickAvatar = async () => {
    ...
    try {
      ...
      setNotice(profileUserError(e, '头像更新失败'))
    } finally {
      setBusy(false)
    }
  }
    ...
    <div className="profile-panel">
        ...
        <div className="profile-hero-card">
          ...
          <input ref={fileRef} hidden type="file" accept="image/*" onChange={e => {
            ...
            void (nativePath ? publishAvatar(nativePath) : readFileAsDataURL(file).then(publishAvatar))
              .catch(err => setNotice(profileUserError(err, '头像更新失败')))
          }} />
      ...
      <section className="setting-group profile-section" data-profile-section="lan" aria-labelledby="profile-lan-title">
        ...
        <div className="profile-stack-row">
          ...
          <button type="button" className={`profile-disc-toggle${profile?.discoveryEnabled ? ' on' : ''}`} disabled={busy} aria-pressed={!!profile?.discoveryEnabled} onClick={() => void people.discoverySet({ enabled: !profile?.discoveryEnabled }).then(d => profile && apply({ ...profile, discoveryEnabled: d.enabled })).catch(e => setNotice(profileUserError(e, '无法切换发现')))}>
        ...
        <div className="profile-pin">
          ...
          <button type="button" disabled={busy} onClick={() => void identity.update({ regeneratePairingCode: true }).then(apply).catch(e => setNotice(profileUserError(e, '无法更换配对码')))}>更换</button>
        </div>
      </section>
      ...
      <section className="setting-group profile-section" data-profile-section="security" aria-labelledby="profile-security-title">
        ...
        {profile?.locked ? (
          <>
            ...
            <div className="profile-actions">
              <button type="button" onClick={() => void identity.unlock({ password: unlock }).then(apply).catch(e => setNotice(profileUserError(e, '解锁失败')))}>解锁</button>
            </div>
          </>
        ) : (
          <>
            ...
            <div className="profile-actions">
              <button type="button" disabled={busy} onClick={() => void identity.passwordSet({ password, currentPassword }).then(next => { apply(next); setPassword(''); setCurrentPassword(''); setNotice(password ? '启动密码已更新' : '启动密码已清除') }).catch(e => setNotice(profileUserError(e, '无法设置密码')))}>保存密码</button>
            </div>
          </>
        )}
      </section>
