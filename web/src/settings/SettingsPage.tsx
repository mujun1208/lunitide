...
import{VoiceDiagnosticsRow}from'./VoiceDiagnosticsRow'
import{RefEngineInstallRow}from'./RefEngineInstallRow'
import{OnnxEngineInstallRow}from'./OnnxEngineInstallRow'
...
import { CompanionSkinSwitch } from '../session/companion/CompanionSkinSwitch'
...
export function CompanionSection():React.JSX.Element{
 ...
 const[prewarmPref,setPrewarmPref]=useState(voicePrewarmPref())
 const prewarmOn=prewarmPref==='on'?true:prewarmPref==='off'?false:prewarmDefaultForEngine(companion.engine)
 ...
 const engineDesc=companion.engine==='onnx'?(engineState==='probing'?'正在获取本地音色…':engineState==='available'?`Kokoro 本地音色 ${voices.length} 种（离线合成，含 8 个中文音色；安装即用，不依赖任何外部路径）`:'本地语音尚未安装，请在下方下载后离线使用'):companion.engine==='edge'?(engineState==='probing'?'正在获取微软云端音色…':engineState==='available'?`普通话音色 ${zhVoices.length} 种（男 ${zhMale} · 女 ${zhFemale}${showForeignEdgeVoices?` · 全部 ${voices.length}`:''}；微软 Edge Neural 风格扩展，免密钥，需联网）`:'无法连接微软云端语音（需联网）'):companion.engine==='natural'?(engineState==='probing'?'正在获取自然语音列表…':engineState==='available'?`自然语音音色 ${voices.length} 个（Windows OneCore 神经网络音色，本机离线合成），按角色风格分组`:'本机无自然语音（M95-001），月伴将自动切换字幕模式'):companion.engine==='ref'?(engineState==='probing'?'正在获取角色音色列表…':refDesc):companion.engine==='volc'?(engineState==='probing'?'正在获取火山官方音色…':engineState==='available'?`火山 seed-tts 2.0 官方音色 ${voices.length} 种（不是本机 50 种人生；密钥配在供应商「语音模型」）`:'火山朗读不可用。请在供应商里配置 Agent Plan 专属 API Key'):engineState==='probing'?'正在检测本机语音合成引擎…':engineState==='available'?`本机可用音色 ${voices.length} 个（Windows SAPI 桌面语音，离线合成）`:'本机无语音合成引擎（M95-001），月伴将自动切换字幕模式'
 ...
 return <><div className="voice-path-heading">语音通道</div><VoicePathPicker value={companion.voicePath} volcTtsReady={volcTtsReady} talkReady={talkReady} onChange={next=>save(applyVoicePath(companion,next,{volcTtsReady}))}/><Toggle label="启用月伴对话" desc="在普通聊天输入框显示月亮按钮，进入全屏语音对话舞台；关闭即回滚入口。月伴建议闪模型：用当前供应商里已有的最快文本模型（air / flash），不要开推理。" on={companion.enabled} onChange={v=>save({...companion,enabled:v})}/><div className="setting-row"><div><div className="setting-label">月伴外观</div><div className="setting-desc">玉盘是现在的月亮；星尘是粒子星河。说「切换风格」也能换。</div></div><CompanionSkinSwitch value={companion.visualSkin==='particle'?'particle':'classic'} onChange={skin=>save({...companion,visualSkin:skin})} zh/></div><HotkeyRow label="打断快捷键" desc="月汐说话或思考时，按此快捷键立刻停止；舞台上也有「打断」按钮。Esc 仍用于退出月伴。" hotkey={companion.interruptHotkey} onChange={hotkey=>save({...companion,interruptHotkey:hotkey})}/><Toggle label="回复自动朗读" desc="边生成边朗读（流式字幕同步更新）；关闭后仅显示字幕。" on={companion.autoSpeak} onChange={v=>save({...companion,autoSpeak:v})}/><Toggle label="先应一声" desc="默认关闭。打开后你说完立刻垫一句「嗯」；真正回答到了会接上。垫音容易被听写当成你的下一句，形成嗯嗯循环。" on={companion.instantAck} onChange={v=>save({...companion,instantAck:v})}/><Toggle label="全双工对话" desc="她说完后立刻接着听下一句，不必重新点麦克风。她思考和说话的整个回合里麦克风都静音，所以外放回声、电视声或旁人说话都不会把她的回答截断；想插话请点舞台上的「打断」或使用快捷键。云端/本地不能对着麦自动插话。" on={companion.fullDuplex} onChange={v=>save({...companion,fullDuplex:v})}/>{(companion.engine==='ref'||companion.engine==='volc')&&<Toggle label="进入即预热合成引擎" desc={companion.engine==='ref'?'本地默认开启。进入月伴时先静默预热本地 SoVITS，消除第一句回答约 8 秒的冷启动等待；关闭则回到冷启动。':'默认关闭。开启后进入月伴时先静默预热火山合成，避免第一句冷启动等待；注意预热会消耗少量套餐额度。'} on={prewarmOn} onChange={v=>{const p=v?'on':'off';setPrewarmPref(p);setVoicePrewarmPref(p)}}/>}<p className="voice-path-hint" role="note">三种通道统一：她思考与说话期间麦克风静音，说完再听你下一句；想打断请点舞台「打断」按钮或快捷键，不会被外放回声/旁人说话/电视声误打断。</p>{companion.voicePath==='volc'&&talkReady&&<Toggle label="实时全双工通话核（火山 · 实验）" desc="默认关闭。关闭时火山走稳健的单声道级联（seed-asr→模型→seed-tts），全程只有一个声音，用「打断」按钮切回；开启后在支持的实时模型上启用端到端全双工通话核，可对着麦自然插话（由服务端 VAD + 回声消除处理）。" on={companion.talkRealtime} onChange={v=>save({...companion,talkRealtime:v})}/>}<Toggle label="嘈杂环境模式" desc="提高麦克风能量门限、延长静音判定，减少旁人说话和背景声误触发。" on={companion.speechEnvironment==='noisy'} onChange={v=>save({...companion,speechEnvironment:v?'noisy':'normal'})}/><LocalAsrRow companion={companion} save={save}/><AsrCorrectionRow/>{shownPath==='local'&&<><div className="setting-row"><div><div className="setting-label">本地引擎</div><div className="setting-desc">Kokoro：离线合成，安装即用，含 8 个中文音色（推荐）。GPT-SoVITS：零样本克隆，需自备/下载引擎包。</div></div><select className="setting-select" aria-label="本地语音引擎" value={companion.engine==='ref'?'ref':'onnx'} onChange={e=>save(applyLocalEngine(companion,e.target.value==='ref'?'ref':'onnx'))}><option value="onnx">Kokoro 离线（推荐）</option><option value="ref">GPT-SoVITS 克隆（高级）</option></select></div>{companion.engine==='ref'?<><VoicePersonaGrid caption="50 种人生已内置。音色走本机克隆引擎，点选即用。" value={companion.voiceId||companion.omniPersonaId} onChange={id=>save({...companion,voiceId:id,omniPersonaId:id})}/><RefEngineInstallRow/><div className="setting-row"><div><div className="setting-label">GPT-SoVITS 服务地址</div><div className="setting-desc">默认 http://127.0.0.1:9880。留空使用默认。</div></div><input className="setting-input" style={{flex:1,fontFamily:'var(--mono)',fontSize:12}} placeholder="http://127.0.0.1:9880" value={companion.refEndpoint} onChange={e=>save({...companion,refEndpoint:e.target.value.trim()})} aria-label="GPT-SoVITS 服务地址"/></div><div className="setting-row"><div><div className="setting-label">{engineDesc}</div></div><button disabled={busy||!refPreviewReady({engineState,hostState:refMeta?.host_state,serverOnline:refMeta?.server_online})} onClick={()=>void preview()}>{refPreviewButtonLabel({busy,launching:hostLaunching})}</button></div></>:<><OnnxEngineInstallRow/><div className="setting-row"><div><div className="setting-label">朗读音色</div><div className="setting-desc">{engineDesc}</div></div><div style={{display:'flex',gap:8,alignItems:'center'}}><select className="setting-select" aria-label="朗读音色" disabled={voiceDisabled} value={companion.voiceId} onChange={e=>save({...companion,voiceId:e.target.value})}><option value="">默认音色</option>{voiceGroups.map(([grp,items])=><optgroup key={grp} label={grp}>{items.map(voice=><option key={voice.voice_id} value={voice.voice_id}>{voice.display_name}</option>)}</optgroup>)}</select><button disabled={busy||engineState!=='available'} onClick={()=>void preview()}>{refPreviewButtonLabel({busy,launching:false})}</button></div></div><div className="setting-row"><div><div className="setting-label">语速</div><div className="setting-desc">当前 {companion.rate}</div></div><input type="range" min={-10} max={10} step={1} value={companion.rate} aria-label="朗读语速" onChange={e=>save({...companion,rate:Number(e.target.value)})} style={{accentColor:'var(--tide1)'}}/></div><div className="setting-row"><div><div className="setting-label">音量</div><div className="setting-desc">当前 {companion.volume}</div></div><input type="range" min={0} max={100} step={1} value={companion.volume} aria-label="朗读音量" onChange={e=>save({...companion,volume:Number(e.target.value)})} style={{accentColor:'var(--tide1)'}}/></div></>}</>}{shownPath==='volc'&&volcTtsReady&&<><VoicePersonaGrid caption="火山 seed-tts 2.0 官方音色。不是本机 50 种人生，也不是豆包 App 温柔桃子。" personas={volcPersonas} filterPlaceholder="查找官方音色" filterLabel="查找火山音色" empty="没有匹配的音色。" value={companion.voiceId||''} onChange={id=>save({...companion,voiceId:id})}/><div className="setting-row"><div><div className="setting-label">{engineDesc}</div></div><button disabled={busy||engineState!=='available'} onClick={()=>void preview()}>{refPreviewButtonLabel({busy,launching:false})}</button></div><div className="setting-row"><div><div className="setting-label">语速</div><div className="setting-desc">当前 {companion.rate}</div></div><input type="range" min={-10} max={10} step={1} value={companion.rate} aria-label="朗读语速" onChange={e=>save({...companion,rate:Number(e.target.value)})} style={{accentColor:'var(--tide1)'}}/></div><div className="setting-row"><div><div className="setting-label">音量</div><div className="setting-desc">当前 {companion.volume}</div></div><input type="range" min={0} max={100} step={1} value={companion.volume} aria-label="朗读音量" onChange={e=>save({...companion,volume:Number(e.target.value)})} style={{accentColor:'var(--tide1)'}}/></div></>}{(shownPath==='cloud'||shownPath==='volc'&&!volcTtsReady)&&<><Toggle label="显示外语音色" desc="默认只列出中文；开启后可浏览全部云端 Neural 音色。" on={showForeignEdgeVoices} onChange={setShowForeignEdgeVoices}/><div className="setting-row"><div><div className="setting-label">朗读音色</div><div className="setting-desc">{engineDesc}</div></div><div style={{display:'flex',gap:8,alignItems:'center'}}><select className="setting-select" aria-label="朗读音色" disabled={voiceDisabled} value={companion.voiceId} onChange={e=>save({...companion,voiceId:e.target.value})}><option value="">默认音色</option>{voiceGroups.map(([grp,items])=><optgroup key={grp} label={grp}>{items.map(voice=><option key={voice.voice_id} value={voice.voice_id}>{voice.display_name}</option>)}</optgroup>)}</select><button disabled={busy||engineState!=='available'} onClick={()=>void preview()}>{refPreviewButtonLabel({busy,launching:false})}</button></div></div><div className="setting-row"><div><div className="setting-label">语速</div><div className="setting-desc">当前 {companion.rate}</div></div><input type="range" min={-10} max={10} step={1} value={companion.rate} aria-label="朗读语速" onChange={e=>save({...companion,rate:Number(e.target.value)})} style={{accentColor:'var(--tide1)'}}/></div><div className="setting-row"><div><div className="setting-label">音量</div><div className="setting-desc">当前 {companion.volume}</div></div><input type="range" min={0} max={100} step={1} value={companion.volume} aria-label="朗读音量" onChange={e=>save({...companion,volume:Number(e.target.value)})} style={{accentColor:'var(--tide1)'}}/></div></>}<VoiceDiagnosticsRow/>{status&&<p role="status" className="notice">{status}</p>}</>
}
