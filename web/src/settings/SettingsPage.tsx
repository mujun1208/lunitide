...
import{hasConfiguredVolcTts}from'../provider/modelKind'
...
export function CompanionSection():React.JSX.Element{
 ...
 useEffect(()=>{let cancelled=false;getProviderBridge().list().then(r=>{if(!cancelled)setVolcTtsReady(hasConfiguredVolcTts(r.items??[]))}).catch(()=>{if(!cancelled)setVolcTtsReady(false)});return()=>{cancelled=true}},[])
