...
function CompanionSection():React.JSX.Element{
 ...
 return <><div className="setting-row" style={{gridTemplateColumns:'1fr'}}><div className="setting-group-title" style={{marginTop:8}}>月伴对话</div></div><Toggle label="启用月伴对话" desc="在普通聊天输入框显示月亮按钮，进入全屏语音对话舞台；关闭即回滚入口。" on={companion.enabled} onChange={v=>save({...companion,enabled:v})}/><Toggle label="回复自动朗读" desc="边生成边朗读（流式字幕同步更新）；关闭后仅显示字幕。" on={companion.autoSpeak} onChange={v=>save({...companion,autoSpeak:v})}/><Toggle label="全双工对话" desc="麦克风在月汐说话时保持开启。外放时她不会被自己的声音打断；想插话请点舞台上的「打断」或使用快捷键。" on={companion.fullDuplex} onChange={v=>save({...companion,fullDuplex:v})}/><HotkeyRow label="打断快捷键" desc="月汐说话或思考时，按此快捷键立刻停止；舞台上也有「打断」按钮。Esc 仍用于退出月伴。" hotkey={companion.interruptHotkey} onChange={hotkey=>save({...companion,interruptHotkey:hotkey})}/><Toggle label="嘈杂环境模式" desc="提高麦克风能量门限、延长静音判定，减少旁人说话和背景声误触发。" on={companion.speechEnvironment==='noisy'} onChange={v=>save({...companion,speechEnvironment:v?'noisy':'normal'})}/><LocalAsrRow companion={companion} save={save}/><div className="setting-row"><div><div className="setting-label">朗读引擎</div><div className="setting-desc">云端免费走微软 Edge 朗读（晓晓等 Neural，免 API Key，需联网）；自然语音为本机 OneCore；本机语音为经典 SAPI；角色扮演音色通过本地 GPT-SoVITS 克隆 50 种音色，服务未运行时自动在后台启动。</div></div><select className="setting-select" aria-label="朗读引擎" value={companion.engine} onChange={e=>save({...companion,engine:e.target.value as CompanionSettings['engine'],voiceId:''})}><option value="edge">云端免费（微软晓晓 · 推荐）</option><option value="natural">自然语音（本机 OneCore）</option><option value="sapi">本机语音（经典 SAPI）</option><option value="ref">角色扮演音色（GPT-SoVITS · 50 种音色）</option></select></div>
