...
it('sends the exact coordination bridge methods and payloads',async()=>{
 ...
 await b.createTodo(create);await b.startRun({runId:U});await b.runTree({planId:U});await b.spawnRun(spawn);await b.joinRun({runId:U,mode:'all'});await b.cancelRun({runId:U});await b.retryRun({runId:U,expectedVersion:2,acknowledgeUncertain:true})
 expect(sent.map(x=>[x.method,x.payload])).toEqual([['plan.todo.create',create],['plan.run.start',{runId:U}],['plan.run.tree',{planId:U}],['plan.run.spawn',spawn],['plan.run.join',{runId:U,mode:'all'}],['plan.run.cancel',{runId:U}],['plan.run.retry',{runId:U,expectedVersion:2,acknowledgeUncertain:true}]])
})
