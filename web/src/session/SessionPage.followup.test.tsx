...
it('keeps prior thinking when 「做好了没有」 attaches to the in-flight task; 停止 still cancels', async () => {
  ...
  render(<SessionPage project={project} bridge={sessionBridge} personal initialSession={session} providers={providers} messages={{ list: vi.fn().mockResolvedValue({ items: [], hasMore: false, nextCursor: null, snapshotSequence: 0 }), append: vi.fn().mockResolvedValue({}) } as MessageBridge} chat={chat} onBack={vi.fn()} />)
  ...
  await user.click(screen.getByRole('button', { name: '↑ 排队补充输入' }))
...
it('queues 「做好了没有」 for an in-flight 报告编写专家 turn without aborting', async () => {
  ...
  render(<SessionPage project={project} bridge={sessionBridge} personal initialSession={session} providers={providers} messages={{ list: vi.fn().mockResolvedValue({ items: [], hasMore: false, nextCursor: null, snapshotSequence: 0 }), append: vi.fn().mockResolvedValue({}) } as MessageBridge} chat={chat} onBack={vi.fn()} />)
  const input = await screen.findByLabelText('向月汐提问，或描述你想完成的任务…')
  fireEvent.change(input, { target: { value: '请 报告编写专家写一份调研报告' } })
  await user.click(screen.getByRole('button', { name: '↑ 发送并对话' }))
  await waitFor(() => expect(start).toHaveBeenCalledOnce())
