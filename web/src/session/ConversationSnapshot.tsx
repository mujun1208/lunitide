...
const safeName=(value:string)=>value.trim().replace(/[<>:"/\\|?*\u0000-\u001f]/g,'-').replace(/[. ]+$/,'').slice(0,80)||'月汐对话'
...
export function ConversationSnapshot({open,title,items,onClose}:{open:boolean;title:string;items:readonly MessageDTO[];onClose:()=>void}){
 ...
 return <div className="snapshot-dialog" role="dialog" aria-modal="true" aria-label="生成对话图片"><div className="snapshot-dialog-head"><div><h2>生成对话图片</h2><p>已选择 {items.length} 条消息。图片只保存在本机或交给系统分享，不会生成虚假的本地分享链接。</p></div><button onClick={onClose} disabled={busy}>取消</button></div><div className="snapshot-preview"><div className="snapshot-canvas" ref={node}><header><b>LUNITIDE · 月汐</b><h1>{title}</h1><time>{new Date().toLocaleString()}</time></header>{items.map(item=><article key={item.id} className={item.role}><div><b>{item.role==='user'?'我':item.role==='assistant'?'月汐':'工具'}</b><time>{new Date(item.createdAt).toLocaleString()}</time></div><section><MarkdownMessage text={item.text}/></section></article>)}</div></div>{notice&&<p className="snapshot-notice" role="status">{notice}</p>}<div className="snapshot-actions"><button onClick={()=>void save()} disabled={busy}>下载图片</button><button onClick={()=>void copy()} disabled={busy}>复制图片</button><button className="primary" onClick={()=>void share()} disabled={busy}>{busy?'生成中…':'系统分享'}</button></div></div>
}
