...
it('fits mermaid SVG so inline height cannot leave a huge empty band', () => {
  ...
  fitMermaidSvg(svg, { fill: true, zoom: 1.5 })
...
it('waits while arrows, quotes or subgraphs are still being typed', () => {
  ...
  expect(mermaidFenceStillOpen('```mermaid\nflowchart TD\nA-->B\n```\n下一步')).toBe(false)
  expect(mermaidTransientError('图表渲染超时，已回收独立渲染进程；源码仍保留')).toBe(true)
  expect(mermaidTransientError('图表渲染已取消，已回收独立渲染进程；源码仍保留')).toBe(true)
  expect(mermaidTransientError('独立图表进程未能启动，源码仍保留')).toBe(false)
