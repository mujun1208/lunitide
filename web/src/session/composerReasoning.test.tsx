...
describe('composer reasoning intensity', () => {
  ...
  test('the top stop is max, which the chat request sends as reasoning effort', () => {
    ...
    const stops = document.querySelectorAll('.composer-reasoning-rail span')
    expect(stops).toHaveLength(3)
    expect(stops[2]).toHaveStyle({ left: '100%' })
    expect(document.querySelector('.composer-reasoning-scale')).toHaveStyle({ '--reasoning-p': '0.5' })
