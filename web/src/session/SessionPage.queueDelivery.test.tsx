...
it.each([undefined, '01ARZ3NDEKTSV4RRFFQ69G5FB8'])('starts the saved queue delivery in its scope %s without appending the long text again', async (officeTaskId) => {
  ...
  const items = [{ queuedId: d, officeTaskId, seq: 1, text: '🙂'.repeat(8000), status: 'queued' as const, mark: 'turn_boundary' as const, createdAt: now }]
  ...
  expect(start.mock.calls[0][0].officeTaskId).toBe(officeTaskId)
  expect(runQueueBridge.consume).toHaveBeenCalledWith({ sessionId: s, ...(officeTaskId ? { officeTaskId } : {}) })
