...
it('does not show raw English plan list or run-tree failures',async()=>{
  render(<CoordinationPlanPanel projectId={P} bridge={{...api(),list:vi.fn().mockRejectedValue(new Error('Failed to fetch'))} as unknown as PlanBridge}/>)
  expect(await screen.findByRole('alert')).toHaveTextContent('计划列表加载失败')
  expect(screen.queryByText(/Failed to fetch/)).toBeNull()
  cleanup()
  render(<CoordinationPlanPanel projectId={P} bridge={{...api(),runTree:vi.fn().mockRejectedValue(new Error('Failed to fetch'))} as unknown as PlanBridge}/>)
  expect(await screen.findByRole('alert')).toHaveTextContent('计划清单载入失败')
  expect(screen.queryByText(/Failed to fetch/)).toBeNull()
})
