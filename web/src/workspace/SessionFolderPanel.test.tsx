...
it('lists directories and files, then previews a file in the workspace instead of opening Office', async () => {
  ...
  vi.mocked(sessionFolderBridge.list).mockImplementation(async ({ relativePath }) => {
    if (!relativePath) {
      return { items: [{ name: '周报', path: '周报', directory: true }, { name: 'notes.txt', path: 'notes.txt', directory: false }], truncated: false }
    }
    return { items: [{ name: '周报_2026-W37.md', path: '周报/周报_2026-W37.md', directory: false }], truncated: false }
  })
