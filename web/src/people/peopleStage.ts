...
export const PEOPLE_STAGE_CHUNK = 32 * 1024

export type PeopleStageProgress = (percent: number, chunk: number, totalChunks: number) => void
...
function stageError(error: unknown, chunk: number, totalChunks: number): Error {
  if (error instanceof BridgeClientError) {
    ...
    if (error.code === 'BRIDGE_REQUEST_TOO_LARGE') {
      return new Error('分片过大，无法通过 Bridge 发送，请换较小图片或改用本机文件选择')
    }
    if (error.code === 'IDENTITY_LOCKED') {
      return new Error('请先解锁本机个人资料后再发送文件')
    }
...
export async function stageBrowserFile(
  people: PeopleBridge,
  file: File,
  ready: Uint8Array,
  onProgress?: PeopleStageProgress,
): Promise<string> {
  const buf = ready
  const uploadId = newUploadId()
  const totalChunks = Math.max(1, Math.ceil(buf.length / PEOPLE_STAGE_CHUNK))
  let offset = 0
  let index = 0
  let localPath = ''
  while (offset < buf.length) {
    const end = Math.min(offset + PEOPLE_STAGE_CHUNK, buf.length)
    ...
    try {
      ...
      if (end === buf.length && !staged.localPath) {
