...
export function Atmosphere({theme,aurora=false}:{theme:Theme;aurora?:boolean}):React.JSX.Element{
 ...
 return <div className="atmosphere" data-aurora={webgl?'webgl':undefined} aria-hidden="true"><div className="sky"/>{webgl&&<div className="atmosphere-aurora"><Aurora {...launchAuroraProps(theme)}/></div>}<div className="nebula"/><div className="stars">{stars.map((s,i)=><i key={i} className={s.bright?'bright':''} style={{left:`${s.left}%`,top:`${s.top}%`,width:`${s.size}px`,height:`${s.size}px`,animationDelay:`${s.delay}s`,animationDuration:`${s.duration}s`}}/>)}</div><div className="vignette"/></div>
}
export function Moon({small=false}:{small?:boolean}):React.JSX.Element{return <span className={`real-moon ${small?'small':''}`} aria-hidden="true" style={{display:'none'}}/>}