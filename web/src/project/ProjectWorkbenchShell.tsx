...
export function ProjectWorkbenchShell({project,projects,sessions,messages,stages,chat,providers,plans=planBridge,reviews=reviewBridge,onBack,onProjectUpdated,initialSession,initialPrompt,initialProviderId,initialModelId}:ShellProps):React.JSX.Element{
 ...
 const projectSidePanel=showSidePanel?(showRelease?<ReleasePanel project={projectState} projects={projects} readOnly={readOnly} onProjectUpdated={updated=>{patchProject(updated);void stages.list({projectId:updated.id}).then(result=>{if(mounted.current)setStageItems(result.items)}).catch(()=>{if(mounted.current)setPhaseWarning('发布准备已保存，阶段列表刷新失败，请重新进入项目核对')})}}/>:showRegistry?<RegistryPanel project={projectState} phase={activePhase} readOnly={readOnly} devGateReady={devGateReady} onGoDevPhase={goDevPhase}/>:<DeliverablePanel project={projectState} phase={activePhase} bridge={projects} stages={stages} stageItems={stageItems} readOnly={readOnly} onProjectUpdated={patchProject} onStagesUpdated={setStageItems} onDeliverablesChanged={()=>void fetchDevGateReady(projectState.id,projectState.type).then(setDevGateReady).catch(()=>setDevGateReady(false))}/>):undefined
