...
export function ProjectWorkbenchShell({project,projects,sessions,messages,stages,chat,providers,onBack,onProjectUpdated,initialSession,initialPrompt,initialProviderId,initialModelId}:ShellProps):React.JSX.Element{
 ...
 const hasSidePanel=(phase:number)=>phase===8||deliverablesForPhase(phase,projectState.type).length>0
 ...
 const[deliverableWidth,startDeliverableResize]=usePanelResize({storageKey:'lunitide:pm-deliverable-width',initial:360,min:280,max:()=>Math.min(520,window.innerWidth-640),reverse:true})
