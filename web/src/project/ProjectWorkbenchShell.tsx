...
import{getDesktopFilesBridge,planBridge,reviewBridge}from'../bridge/client'
...
import{ProjectDevBar,hubWorkspaceRoot,openProjectTask}from'./ProjectDevBar'
import{projectSpineApi,shortRootPath,type ProjectExecutor,type TaskBrief}from'./projectSpineApi'
import{isDevWorkflowPhase}from'./devWorkflowChips'
...
export function ProjectWorkbenchShell({project,projects,sessions,messages,stages,chat,providers,plans=planBridge,reviews=reviewBridge,onBack,onProjectUpdated,initialSession,initialPrompt,initialProviderId,initialModelId}:ShellProps):React.JSX.Element{
 ...
 const[currentTargetPath,setCurrentTargetPath]=useState('')
 const[hubThreadId,setHubThreadId]=useState('')
 const[hubThreads,setHubThreads]=useState<Record<string,string>>({})
 ...
 // Soft completion lock (Issue 4c): moving into a later phase while the phase
 // right before it has not passed its three-gate晋级 is allowed, but we say so —
 // a dismissible warning, never a hard block — so the衔接 gap is visible instead
 // of silently skipped. Stage status is the source of truth (finalize marks the
 // prior stage completed on晋级).
 const priorPhaseIncomplete=(phase:number):string=>{const idx=phaseDefs.findIndex(d=>d.phase===phase);if(idx<=0)return '';const prior=phaseDefs[idx-1];const s=stageMap.get(prior.phase)?.status;if(s==='completed'||s==='approved')return '';return `上一阶段「${prior.label}」还没完成三关确认。可以继续，但请留意上下游衔接。`}
 const selectPhase=(phase:number)=>{setActivePhase(phase);writePreferredPhase(projectState.id,phase);setSidePanelOpen(hasSidePanel(phase));setPhaseWarning(priorPhaseIncomplete(phase))}
 ...
 useEffect(()=>{if(!projectState.rootPath)return;void projectSpineApi.treeGet({projectId:projectState.id}).catch(()=>{})},[projectState.id,projectState.rootPath])
 const goDevPhase=()=>selectPhase(devPhaseForType(projectState.type))
 const goReqPhase=()=>selectPhase(1)
 const ifaceStage=stageMap.get(interfaceRegistryPhase(projectState.type))
 const interfaceReady=!!ifaceStage&&(ifaceStage.status==='completed'||ifaceStage.status==='approved')
 const onWorkbenchStats=useCallback((stats:WorkbenchStats)=>{setWorkbenchStats(stats);if(stats.lastAssistant!==undefined)setLastAssistant(stats.lastAssistant)},[])
 const openTask=async(itemId:string,rowExecutor?:ProjectExecutor)=>{
  try{
   const opened=await openProjectTask(projectState,itemId,rowExecutor,session?.id)
   setCurrentTaskId(opened.brief.itemId)
   setCurrentBrief(opened.brief.text)
   setCurrentTargetPath(opened.brief.targetRelPath)
   goDevPhase()
   if(opened.executor!=='lunitide'){
    const hub=await import('../agentHub/agentHubApi').then(({agentHubApi})=>agentHubApi)
    const existing=opened.hubThreadId||hubThreads[itemId]
    if(existing){
     setHubThreadId(existing)
     setHubThreads(prev=>({...prev,[itemId]:existing}))
     if(opened.brief.text)await hub.threadPrompt({threadId:existing,text:opened.brief.text})
    }else{
     const created=await hub.threadCreate({
      harnessId:opened.executor,
      scene:'write_project',
      projectId:projectState.id,
      workspaceRoot:hubWorkspaceRoot(projectState),
      title:`${opened.brief.itemId} ${opened.brief.title}`.slice(0,200),
      accessMode:'approval',
     })
     setHubThreadId(created.thread.threadId)
     setHubThreads(prev=>({...prev,[itemId]:created.thread.threadId}))
     if(opened.brief.text)await hub.threadPrompt({threadId:created.thread.threadId,text:opened.brief.text})
     await projectSpineApi.taskOpen({projectId:projectState.id,itemId,hubThreadId:created.thread.threadId,workSessionId:session?.id})
    }
   }
  }catch(err){setPhaseWarning(err instanceof Error?err.message:'无法打开开发任务')}
 }
 const pickMissingRoot=async()=>{
  if(rebindBusy)return
  setRebindBusy(true)
  try{
   const picked=await projectSpineApi.rootPick()
   if(picked.canceled||!picked.path)return
   const saved=await projectSpineApi.rootRebind({id:projectState.id,version:projectState.version,rootPath:picked.path})
   patchProject(saved)
  }catch(err){setPhaseWarning(err instanceof Error?err.message:'补选项目根失败')}
  finally{setRebindBusy(false)}
 }
 const materializeTree=async()=>{
  if(rebindBusy||!projectState.rootPath)return
  setRebindBusy(true)
  try{
   const saved=await projectSpineApi.treeMaterialize({projectId:projectState.id,version:projectState.version})
   patchProject(saved.project)
  }catch(err){setPhaseWarning(err instanceof Error?err.message:'补生成目录失败')}
  finally{setRebindBusy(false)}
 }
 const workspaceTab=workspaceTabForPhase(activePhase,activePhaseDef?.label)
 const isDevPhase=isDevWorkflowPhase(activePhase,activePhaseDef?.label)
 const deliverablePanel=deliverablesForPhase(activePhase,projectState.type).length>0?<DeliverablePanel project={projectState} phase={activePhase} bridge={projects} stages={stages} stageItems={stageItems} readOnly={readOnly} onProjectUpdated={patchProject} onStagesUpdated={setStageItems} onOpenTask={(itemId,executor)=>void openTask(itemId,executor)} onSelectBoardItem={(itemId,brief)=>{setCurrentTaskId(itemId);if(brief)setCurrentBrief(brief)}} currentTaskId={currentTaskId} onGoDevItem={itemId=>void openTask(itemId)} expertCount={expertCount} onPrefillPrompt={text=>{setCurrentBrief(text);window.dispatchEvent(new CustomEvent('lunitide:composer-prefill',{detail:{text}}))}}/>:undefined
 const registryPanel=showRegistry?<RegistryPanel project={projectState} phase={activePhase} readOnly={readOnly} registryReady={registryTreeReady(projectState)} onGoReqPhase={goReqPhase}/>:undefined
 const docsPanel=showSidePanel?(showRelease?<ReleasePanel project={projectState} projects={projects} readOnly={readOnly} onProjectUpdated={updated=>{patchProject(updated);void stages.list({projectId:updated.id}).then(result=>{if(mounted.current)setStageItems(result.items)}).catch(()=>{if(mounted.current)setPhaseWarning('发布准备已保存，阶段列表刷新失败，请重新进入项目核对')})}}/>:<div>{deliverablePanel}{registryPanel}</div>):undefined
 const projectSidePanel=isDevPhase?<div>{<ProjectDevBar project={projectState} sessionId={session?.id} currentBrief={currentBrief} currentItemId={currentTaskId} hubThreadId={hubThreadId} lastAssistant={lastAssistant} interfaceReady={interfaceReady} onProjectUpdated={patchProject} onTaskOpened={(brief:TaskBrief)=>setCurrentBrief(brief.text)} onHubThread={setHubThreadId}/>}{docsPanel}</div>:docsPanel
