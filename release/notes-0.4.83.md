...
已装 0.4.81 及更早版本没有下载器，**第一次**仍须手动安装本版。之后才能在线点升级。`v0.4.83` 的 Setup + `latest.json` 由 `publish-update-feed` 挂到 GitHub Latest。
...
- 打 tag `v0.4.83` 后，`publish-update-feed` 在 `windows-latest` 上打包并上传 Setup + `latest.json` + `SHA256SUMS.txt`，标成 Latest。云端没有发布者证书时是未签名包；本机有证书再打签名包应升版本号，不要覆盖同一 digest。
