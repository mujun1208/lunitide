...
foreach($item in $files){
  ...
  if(-not $allowed){throw "Staged file is not on the exact release allowlist: $rel"}
	if($rel.StartsWith('web/dist/',[StringComparison]::Ordinal) -and $rel -notmatch '(?i)\.(html|js|mjs|css|map|json|png|jpe?g|gif|webp|svg|ico|woff2?|ttf|pfb|bcmap)$'){throw "Renderer asset type is not allowed: $rel"}
