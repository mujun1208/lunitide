...
func claimGatewayInstance() (already bool, release func()) {
	...
	if err != nil {
		// Fail closed: refusing to start is safer than running without a mutex.
		return true, func() {}
	}
	handle, _, callErr := procCreateMutexW.Call(0, 0, uintptr(unsafe.Pointer(name)))
	if handle == 0 {
		// CreateMutexW failed (OOM, ACL, etc.). Pretending "not already
		// running" would allow multiple desktops; refuse instead.
		return true, func() {}
	}
