...
// renderMoonMark draws a large pale moon with a cloud stroke underneath.
// The canvas is transparent outside those two marks.
func renderMoonMark(size int) *image.RGBA {
	if size < 16 {
		size = 16
	}
	dst := image.NewRGBA(image.Rect(0, 0, size, size))
	drawMoon(dst, float64(size)*0.50, float64(size)*0.38, float64(size)*0.32)
	cloudY := float64(size) * 0.80
