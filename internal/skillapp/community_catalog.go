...
func communityTemplate(pkg CommunityPackage) (CatalogTemplate, error) {
	...
	if pkg.ID == "find-skills" {
		intro += "\n发现顺序：skill.list 查看已安装，skill.catalog.list 查产品目录；匹配现有版本就复用，缺失才 skill.install，templateId 写目录 id，再 skill.publish。要直接按正文做，用 skill.invoke，skillId 写目录 id。外部社区先核实官方仓库、固定 commit、许可与完整资源，经产品导入后再试用。安装结果必须来自 Lunitide 回执，npx skills 的其他宿主安装结果无效。\n"
	}
	if pkg.ID == "gstack" {
...
func nativeCommunityAlternatives() []CatalogTemplate {
	entries := []struct {
		...
		{"pptx", "PPT演示文稿生成与修改（Lunitide原生）", "根据受众和用途组织每页一个主张，提供实际内容、数据来源及演讲备注。使用pptx.gen或office.generate；没有合适模板就按 cover、agenda、metrics、comparison、quote、timeline、closing 这些版式成稿，不要问风格或品牌。已有稿件使用office.inspect/office.patch保留其余页面。需要增删幻灯片时，可在 MCP 中心连接 PPT MCP，禁止 Office COM 和 command.run 拼文件。核验页面内容和实际生成文件，再提供打开路径；未做渲染检查就明确说明。", []string{"PPT", "pptx", "幻灯片"}},
