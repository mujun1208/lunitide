...
func (e *Engine) skillToolDefinitions() []llmadapter.ToolDefinition {
	...
	return []llmadapter.ToolDefinition{
		...
		{Name: "skill.publish", Description: "Publish one library skill so skill.invoke can run the installed copy. id may be the skill ULID or a catalog template id. Publishing a catalog id installs it first. If the skill is already published and its manifest version is newer than the Skill Center label, this replaces that label in place.", Schema: []byte(`{"type":"object","properties":{"id":{"type":"string","minLength":1,"maxLength":128},"skillId":{"type":"string","maxLength":128}},"additionalProperties":false}`)},
	}
}
