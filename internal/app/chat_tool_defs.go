...
func engineToolDefinitions() []llmadapter.ToolDefinition {
	return []llmadapter.ToolDefinition{
		...
		{Name: "desktop.browse", Description: "Open an HTTP(S) URL or search query in the user's real system-default desktop browser, not the managed automation browser. Omit both to open the search homepage. Opening is acknowledged, not proof of page load; verify the actual browser with computer.act. Never claim web research results from this tool alone.", Schema: []byte(`{"type":"object","properties":{"url":{"type":"string","maxLength":8192},"query":{"type":"string","maxLength":2000}},"additionalProperties":false}`)},
		...
		{Name: "media.play", Description: "Play, pause, or skip music on this machine. target=foreground launches the named desktop player if needed. For any song use query=random; never invent a song title or search for popular music. Generic playback and transport controls target that app's Windows media session and verify playback state. A verified result is sufficient: stop operating and report it. Named song/artist queries use the player's search. Never click favorites/like toggles. target=browser is only for an explicitly requested web player. Full-access mode is enough; the full-disk switch is not required.", Schema: []byte(`{"type":"object","properties":{"action":{"type":"string","enum":["play","open_and_play","open","pause","toggle","next","prev","stop"],"description":"default play"},"query":{"type":"string","description":"exact requested song/artist, or random for any song; omit to resume"},"url":{"type":"string","description":"direct http(s) music page"},"target":{"type":"string","enum":["auto","foreground","browser","netease","qqmusic"],"description":"foreground=desktop player on this PC; auto prefers session context"},"app":{"type":"string","description":"app name to focus when target=foreground"}},"additionalProperties":false}`)},
