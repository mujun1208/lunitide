...
func handleCollabGateEvaluate(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if decodePayload(r.Payload, &p) != nil {
		return bridge.Failure(r.ID, r.TraceID, "BRIDGE_SCHEMA_INVALID", "collabGate.evaluate 参数无效", false)
	}
	if e.m8gate == nil {
		return bridge.Failure(r.ID, r.TraceID, "STORAGE_UNAVAILABLE", "写协作门禁服务暂时不可用", true)
	}
...
func handleCollabGateStatus(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if decodePayload(r.Payload, &p) != nil {
		return bridge.Failure(r.ID, r.TraceID, "BRIDGE_SCHEMA_INVALID", "collabGate.status 参数无效", false)
	}
	if e.m8gate == nil {
		return bridge.Failure(r.ID, r.TraceID, "STORAGE_UNAVAILABLE", "写协作门禁服务暂时不可用", true)
	}
...
func handleCollabGateConfirm(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if decodePayload(r.Payload, &p) != nil {
		return bridge.Failure(r.ID, r.TraceID, "BRIDGE_SCHEMA_INVALID", "collabGate.confirm 参数无效", false)
	}
	if e.m8gate == nil {
		return bridge.Failure(r.ID, r.TraceID, "STORAGE_UNAVAILABLE", "写协作门禁服务暂时不可用", true)
	}
...
func m8CollabGateFailure(r bridge.Request, err error) bridge.Response {
	...
	case errors.Is(err, m8app.ErrGateDisabled):
		return bridge.Failure(r.ID, r.TraceID, "M8-028", "写协作能力未启用，操作被拒绝", false)
	case errors.Is(err, m8app.ErrDecisionTokenInvalid):
		return bridge.Failure(r.ID, r.TraceID, "M8-031", "决策令牌无效、已过期或已重放", false)
	case errors.Is(err, m8app.ErrGateBindingDrift):
		return bridge.Failure(r.ID, r.TraceID, "M8-032", "能力绑定已漂移，写协作已回退为停用", false)
	case errors.Is(err, m8app.ErrEvidenceUnavailable):
		return bridge.Failure(r.ID, r.TraceID, "M8-033", "评估证据源不可用，评估已拒绝（fail-closed）", true)
	case errors.Is(err, m8app.ErrDecisionNotFound):
		return bridge.Failure(r.ID, r.TraceID, "BRIDGE_NOT_FOUND", "决策记录不存在", false)
