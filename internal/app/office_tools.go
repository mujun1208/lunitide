...
func officeToolDefinitions() []llmadapter.ToolDefinition {
	...
	metricItem := obj(map[string]any{"label": boundedText(80), "value": boundedText(64), "unit": boundedText(32), "factId": boundedText(64)}, "label", "value")
	evidenceItem := obj(map[string]any{"text": boundedText(300), "factId": boundedText(64)}, "text")
	slide := obj(map[string]any{"title": boundedText(100), "subtitle": boundedText(300), "layout": map[string]any{"enum": []string{"cover", "section", "content", "two-column", "comparison", "quote", "metrics", "timeline", "agenda", "closing", "table", "conclusion", "trend", "structure", "process", "evidence"}, "description": "metrics and timeline allow at most 6 items; overflow is split, never truncated. table requires rows and no bullets. Rows are forbidden in other layouts."}, "bullets": arr(boundedText(300), 12), "notes": text, "rows": arr(arr(boundedText(160), 6), 9), "purpose": boundedText(200), "claim": boundedText(300), "metrics": arr(metricItem, 12), "comparison": obj(map[string]any{"left": arr(boundedText(300), 8), "right": arr(boundedText(300), 8)}), "evidence": arr(evidenceItem, 8)}, "title")
	slide["description"] = "PPT content is never silently truncated. schemaVersion 2 uses metrics[{label,value,unit,factId}] and comparison.{left,right}; do not encode metrics as value|label and do not invent savings percentages. table rows must be rectangular with 1–9 rows and 1–6 columns. Split oversized content into additional slides."
	...
	fact := obj(map[string]any{"factId": boundedText(64), "value": boundedText(64), "unit": boundedText(32), "period": boundedText(64), "locator": boundedText(80), "locked": map[string]any{"type": "boolean"}}, "factId", "value")
	spec := obj(map[string]any{"schemaVersion": map[string]any{"type": "integer", "enum": []any{1, 2}}, "kind": map[string]any{"enum": []string{"pptx", "docx", "xlsx", "pdf"}}, "title": text, "brandId": text, "templateId": text, "facts": arr(fact, 64), "slides": arr(slide, 30), "blocks": arr(block, 500), "sheets": arr(sheet, 16), "body": text, "document": document}, "schemaVersion", "kind", "title")
	spec["description"] = "Title must be nonempty and at most 1,024 UTF-8 bytes. schemaVersion 1 or 2; v2 adds semantic metrics/comparison and templateId (research-report/client-proposal/product-project or ops-ledger/sales-pipeline/project-tracker). Optional facts[] is the shared FactSet; locked facts must appear unchanged in the generated file. Supply only matching content: pptx requires 1–30 slides; docx requires 1–500 blocks and optional document settings; xlsx requires 1–16 sheets with 1–5,000 rows each, at most 128 columns and 50,000 total cells, unless a workbook templateId fills empty sheets; pdf uses body. Omit content fields belonging to other kinds. Never invent weekly-report rates or savings percentages. Imported files are not rebuilt from this spec."
	...
	return []llmadapter.ToolDefinition{
		...
		definition("office.deliver", "Manage source-backed metrics and fixed-version delivery bundles. capture reads an exact inspected node, or locates a locked fact with factId+value (never invent rawValue, never capture formula cells); apply inserts a metric using template with exactly one {{value}}. metrics pages with offset=nextOffset and textOffset=nextTextOffset; metricId selects one metric for reading long values. bundle fixes explicit versionIds and returns its id. bundles pages summaries with offset, or bundleId plus fileOffset=nextFileOffset to read every fixed file. Outputs stay within the tool budget. Source updates mark drafts stale; accepted versions remain unchanged.", obj(map[string]any{"taskId": text, "action": map[string]any{"enum": []string{"metrics", "capture", "apply", "bundles", "bundle"}}, "offset": map[string]any{"type": "integer", "minimum": 0}, "textOffset": map[string]any{"type": "integer", "minimum": 0}, "fileOffset": map[string]any{"type": "integer", "minimum": 0}, "bundleId": text, "versionId": text, "nodeId": text, "nodeDigest": text, "factId": text, "value": text, "name": text, "unit": text, "currency": text, "period": text, "roundingDigits": map[string]any{"type": "integer", "minimum": 0, "maximum": 12}, "metricId": text, "targetVersionId": text, "targetNodeId": text, "targetNodeDigest": text, "template": text, "expectedRevision": map[string]any{"type": "integer", "minimum": 1}, "title": text, "versionIds": arr(text, 32)}, "action")),
	}
}
	...
	if name == "office.deliver" {
		...
		case "capture":
			if strings.TrimSpace(p.FactID) != "" {
				result, err = s.CaptureMetricByFact(ctx, task.ID, p.VersionID, content.Fact{FactID: p.FactID, Value: p.Value, Unit: p.Unit, Period: p.Period, Locator: p.Name}, key)
			} else {
