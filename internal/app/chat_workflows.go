...
// The *.gen desktop=true / no command.run clause is only added when the
// user asked to generate a file (PPT/Excel/报告/小说), never for play,
// open, type, or weather.
func bundledWorkflowInjection(turnText ...string) string {
	...
	return "\n\n[内置工作流] 开箱即用，直接调用已有工具，不必安装技能、不必 MCP：\n" +
		...
		"- 在已打开的 Word/文档/对话框里填写：desktop.type（after=文档里真实字段名如身份证号码或证件号码，text=要写的内容，需要发送时 submit=true，window=窗口标题）。写完不要关窗口。再打开刚才的文档继续用 desktop.open。找不到字段必须对用户说无法执行和原因。\n" +
		"- 播放音乐/视频：打开桌面播放器后用 media.play target=foreground（没说歌名或要随机播放时 query=热门；说了歌手如周杰伦则 query=周杰伦）。未运行则 desktop.open 启动后再 foreground 搜索播放。禁止点「我喜欢的音乐」「收藏」。成功以正在播放为准，不要只启动进程。禁止默认打开 music.163.com / YouTube。仅当用户明确要网页版时才用 target=browser。\n" +
		"- 暂停/下一首：media.play action=pause|next|prev。\n" +
		"- 发飞书/企微/钉钉/微信/QQ：设置 → 消息通道启用后用 im.send。\n" +
		...
		"- 窗口：cc.window_list 列出，cc.window_focus 激活已运行应用；输入前先 cc.window_focus（或 keyboard_type 的 window=）。未运行的用 desktop.open。用户没说关闭时不要 close。最小化/还原/移动用 cc.window_action；退出应用用 cc.app_quit（禁止关资源管理器/UAC）。拖拽用 cc.mouse_drag。粘贴用 cc.paste；按键用 cc.press；菜单用 cc.menu_click；填表用 cc.set_value。UI 动画未结束用 cc.wait until=change。\n" +
