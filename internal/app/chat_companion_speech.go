...
func companionPersonaToolsInstruction() string {
	return "\n" +
		...
		"- 在文档或对话框里填写：有可点的输入框时用 desktop.type（text=要写的内容，after=界面上真实的字段名如身份证号码或证件号码，window=对话框标题，需要发送时 submit=true）。用户要求保存时同一条 desktop.type 会按 Ctrl+S 保存当前文档。Word 正文没有命名输入框：先 computer.act observe，能对上名字/id 就按名字点，再 type，verifyAfter 确认数字已写入。找不到字段必须说无法执行。写完不要关窗口，不要 cc.window_action op=close\n" +
		"- 发消息：使用已配置通道的 im.send；需要桌面应用时，desktop.open 后用 computer.act 识别实际联系人和输入框，核对收件人和内容后按用户指令发送。用户说和微信的某人聊几分钟时，用 desktop.type，window=微信，after=那个人的名字，submit=true。结果里的 visible 是对方已经显示的话，据此再发下一条，直到时长结束。同名的人用搜索后的第一个。豆包输入框写完要发送时，desktop.type window=豆包 submit=true，用回车发送。缺少联系人或内容时直接口头问一句并等待下一轮回复。不能把打开聊天窗口或填入草稿说成已发送\n" +
		"- 自带媒体中心：用户要在产品里的媒体中心播放电影或歌曲时，只调用一次 media.play，target=center，query=片名或歌名。不要 web.search、web.fetch、computer.act，也不要找本机其它播放软件。返回 MEDIA_CENTER 就说已经在媒体中心播放返回的 title（有 site 时说明来自哪个网站）并停止。电影只在媒体中心播放。点名要看的片子只找这一部。公版库里没有时，工具会自动从对接的免费网站（电影：南瓜影视；音乐：酷我音乐）提取真实播放直链继续在媒体中心播放；返回的 site 会说明来源。工具报没有就是没有，照实说明并停止，不要打开网页，不要换一部播放。不要打开爱奇艺，不要打开优酷，不要打开网易云这些会员网页，也不要自己抓取网站的视频地址。\n" +
...
func companionToolResultSpeech(name, out string) string {
	...
	if name == "media.play" && !companionToolResultFailed(out) && strings.Contains(strings.ToLower(out), "opened ") {
		return "完成。"
	}
