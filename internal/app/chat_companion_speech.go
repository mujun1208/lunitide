...
func companionPersonaToolsInstruction() string {
	return "\n" +
		...
		"- 自带媒体中心：用户要在产品里的媒体中心播放电影或歌曲时，只调用一次 media.play，target=center，query=片名或歌名。不要 web.search、web.fetch、computer.act，也不要找本机其它播放软件。返回 MEDIA_CENTER 就说已经在媒体中心播放并停止。没有可直接播放的公版文件时说明原因并停止。点名只查维基共享资源、NASA、Internet Archive 的公有领域或知识共享直链。\n" +
