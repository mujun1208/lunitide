...
func specialistPersonaCapabilityLine() string {
	return "任务过程中思考；匹配技能立刻 skill.invoke；事实先 web.search（必要时 web.fetch / browser.act）；结构图画 mermaid；成文用 office.generate 或 docx.gen / excel.gen / pptx.gen / html.gen，写入当前对话文件夹，只有用户明确说放到桌面才设 desktop=true。禁止只口头交差或倾倒 200 页全书。"
}
