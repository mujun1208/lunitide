...
func TestParallelToolEligible(t *testing.T) {
	...
	mutating := []string{"workspace.write", "workspace.edit", "command.run", "todo.write", "user.ask", "excel.gen", "docx.gen", "pptx.gen", "pdf.gen", "html.gen", "desktop.open", "desktop.type", "im.send", "cc.mouse_click", "cc.keyboard_type", "subagent.spawn", "plan.run", "data.process", "image.batch", "pdf.copy", "unknown.tool"}
