...
func (e *Engine) prepareSkillTrials(ctx context.Context, session string, ids []string, companion bool) (string, error) {
	...
	b.WriteString("\n[用户本轮显式选择的草稿试用]\n以下草稿尚未发布；执行本轮工作前，先分别 skill.try 读取完整约定。这里的试用优先于同名已发布目录项。不可用 skill.invoke 代替，不得自动发布；其他未选草稿不允许试用。工具权限仍依当前执行模式。试用输出不代表已安装或已通过质量测试。skill.try 一次成功后直接按约定交付。只有用户明确要的 PPT、Word、Excel、PDF 才用 office.generate 的 {name,spec}（本轮若仍有 docx.gen/excel.gen 也可用）。「可以演示的系统 / 页面可操作 / 数据交互 / 数据存储 / POC」是可运行程序，不是幻灯片，禁止 pptx.gen 和 office.generate。源码和脚本用 workspace.write 写到工作区相对路径，再用 command.run 运行。本地服务由 command.run 留在本机并返回地址，禁止要求用户把命令粘贴到外面执行，禁止说没有终端或没有写盘通道。命令或写文件若被拒绝，原样说明原因后改路径重试，不要换成交付 PPT。禁止再 workspace.read 同一份 SKILL.md。\n")
	for _, id := range ids {
