...
func TestCheckAdoptsLocalFeedWhenLedgerEmpty(t *testing.T) {
	...
	h.svc.SetFeedLookup(func(channel string) (string, string, bool, error) {
		...
		return "0.4.83", digest, true, nil
	})
	got, err := h.svc.Check(context.Background(), "stable", "0.4.81")
	if err != nil {
		t.Fatal(err)
	}
	if got.UpdateID == "" || got.Version != "0.4.83" || got.Digest != digest || got.Mandatory {
	...
	current, err := h.svc.Check(context.Background(), "stable", "0.4.83")
	if err != nil {
		t.Fatal(err)
	}
	if current.UpdateID != "" {
