...
func NewVoiceService(root, modelID string) *VoiceService {
	...
	return &VoiceService{
		backend:   &voice.SherpaBackend{Root: root, ModelID: modelID},
...
// Close releases every open session and stops the child process.
...
func handleVoiceStatus(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	ready := e.voice.backend.Ready(ctx) == nil
	return bridge.Success(r.ID, map[string]any{
		"supported":  true,
		"ready":      ready,
		"modelId":    model.ID,
		...
		"downloadBytes": model.TotalBytes() + voice.Runtime().TotalBytes(),
