...
func handleImChannelsSet(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if p.TestSend != nil && *p.TestSend {
		...
		if url == "" {
			return bridge.Failure(r.ID, r.TraceID, "BRIDGE_SCHEMA_INVALID", "试发需要 Webhook 地址", false)
		}
		...
		if err := n.Notify(imapp.Label(kind), "月汐已连上"); err != nil {
			return bridge.Failure(r.ID, r.TraceID, "WEBHOOK_UNREACHABLE", "试发失败，地址没保存："+err.Error(), true)
		}
	}
	if p.ProbeDesktop != nil && *p.ProbeDesktop && p.Enabled != nil && *p.Enabled && (kind == imapp.KindWeChat || kind == imapp.KindQQ) {
		if err := toolruntime.ProbeDesktopApp(imapp.DesktopApp(kind)); err != nil {
			return bridge.Failure(r.ID, r.TraceID, "DESKTOP_APP_MISSING", "本机没有检测到"+imapp.Label(kind)+"，无法启用", false)
		}
	}
	...
	if err != nil {
		...
		if errors.Is(err, imapp.ErrWebhookRequired) {
			return bridge.Failure(r.ID, r.TraceID, "BRIDGE_SCHEMA_INVALID", "飞书/企微/钉钉启用前请粘贴 Webhook，不能只用本机打字", false)
		}
		if errors.Is(err, imapp.ErrInboundAllowlist) {
			return bridge.Failure(r.ID, r.TraceID, "BRIDGE_SCHEMA_INVALID", "开启入站需要 App ID，第一条消息会写入白名单", false)
		}
