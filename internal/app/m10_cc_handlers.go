...
func handleCcGetConfig(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if decodePayload(r.Payload, &p) != nil {
		return r.Fail("BRIDGE_SCHEMA_INVALID", "cc.getConfig 参数无效", false)
	}
	if e.ccctrl == nil {
...
func handleCcUpdateConfig(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if decodePayload(r.Payload, &p) != nil {
		return r.Fail("BRIDGE_SCHEMA_INVALID", "cc.updateConfig 参数无效", false)
	}
	if e.ccctrl == nil {
		return r.Fail("STORAGE_UNAVAILABLE", "电脑控制服务暂时不可用", true)
	}
...
func handleCcGetAuditLog(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if decodePayload(r.Payload, &p) != nil {
		return r.Fail("BRIDGE_SCHEMA_INVALID", "cc.getAuditLog 参数无效", false)
	}
	if e.ccctrl == nil {
		return r.Fail("STORAGE_UNAVAILABLE", "电脑控制服务暂时不可用", true)
	}
	...
	return r.Ok(map[string]any{"items": entries})
}
...
func handleCcEmergencyStop(e *Engine, ctx context.Context, r bridge.Request) bridge.Response {
	...
	if decodePayload(r.Payload, &p) != nil {
		return r.Fail("BRIDGE_SCHEMA_INVALID", "cc.emergencyStop 参数无效", false)
	}
	if e.ccctrl == nil {
		return r.Fail("STORAGE_UNAVAILABLE", "电脑控制服务暂时不可用", true)
	}
...
func ccFailure(r bridge.Request, err error) bridge.Response {
	...
	case "M10-CC-002":
		return r.Fail("M10-CC-002", "电脑控制状态迁移非法（紧急停止后需重新启用）", false)
	case "M10-CC-003":
		return r.Fail("M10-CC-003", "电脑控制审计记录不存在", false)
	case "M10-CC-004":
		return r.Fail("M10-CC-004", "操作被风险策略拦截", false)
	case "M10-CC-005":
		return r.Fail("M10-CC-005", "紧急停止已激活", false)
	case "M10-CC-006":
		return r.Fail("M10-CC-006", "电脑控制操作频率超限", false)
	case "M10-CC-007":
		return r.Fail("M10-CC-007", "操作需要人工确认", false)
	case "M10-CC-008":
		return r.Fail("M10-CC-008", "输入参数被过滤层拒绝", false)
	case "M10-CC-009":
		return r.Fail("M10-CC-009", "前台进程在阻止名单中", false)
	case "M10-CC-010":
		return r.Fail("M10-CC-010", "控制引擎不可用（非 Windows 主机）", false)
	case "M10-CC-011":
		return r.Fail("M10-CC-011", "控制操作执行失败", false)
	case "M10-CC-012":
		return r.Fail("M10-CC-012", "电脑控制未启用", false)
	}
	return r.Fail("STORAGE_UNAVAILABLE", "电脑控制存储暂时不可用", true)
}
