...
// maxPPTXGeneratedBytes is higher because a cloned designer deck still
// carries the pictures on the pages that were kept.
const maxPPTXGeneratedBytes = 32 << 20

func generatedByteLimit(relPath string) int {
	if strings.EqualFold(filepath.Ext(relPath), ".pptx") {
		return maxPPTXGeneratedBytes
	}
	return maxGeneratedBytes
}
