...
var expectedColumns = map[string][]columnSpec{
	...
	"projects":                          {{"id", "TEXT", "", 0, 1, 0}, {"name", "TEXT", "", 1, 0, 0}, {"project_code", "TEXT", "", 1, 0, 0}, {"project_type", "TEXT", "'implementation'", 1, 0, 0}, {"description", "TEXT", "''", 1, 0, 0}, {"summary", "TEXT", "''", 1, 0, 0}, {"objective", "TEXT", "''", 1, 0, 0}, {"client", "TEXT", "''", 1, 0, 0}, {"contract_no", "TEXT", "''", 1, 0, 0}, {"amount", "REAL", "0", 1, 0, 0}, {"budget", "REAL", "0", 1, 0, 0}, {"plan_start", "TEXT", "''", 1, 0, 0}, {"plan_end", "TEXT", "''", 1, 0, 0}, {"remark", "TEXT", "''", 1, 0, 0}, {"close_reason", "TEXT", "''", 1, 0, 0}, {"status", "TEXT", "'active'", 1, 0, 0}, {"created_at", "TEXT", "", 1, 0, 0}, {"updated_at", "TEXT", "", 1, 0, 0}, {"version", "INTEGER", "1", 1, 0, 0}, {"org_id", "TEXT", "", 0, 0, 0}, {"space_id", "TEXT", "", 0, 0, 0}},
...
func (s *Store) ListProjects(ctx context.Context, filter project.Filter) ([]project.Project, error) {
	query := `SELECT id,name,project_code,project_type,description,summary,objective,client,contract_no,amount,budget,plan_start,plan_end,remark,close_reason,status,created_at,updated_at,version,org_id,space_id FROM projects`
	args := []any{}
	conditions := []string{}
	if filter.OrgID != "" {
...
func scanProject(rows *sql.Rows) (project.Project, error) {
	...
	if err := rows.Scan(&p.ID, &p.Name, &p.ProjectCode, &p.Type, &p.Description, &p.Summary, &p.Objective, &p.Client, &p.ContractNo, &p.Amount, &p.Budget, &p.PlanStart, &p.PlanEnd, &p.Remark, &p.CloseReason, &p.Status, &created, &updated, &p.Version, &orgID, &spaceID); err != nil {
		return p, err
	}
	var err error
	if p.CreatedAt, err = time.Parse(time.RFC3339Nano, created); err != nil {
