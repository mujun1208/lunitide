// router_test.go pins the engine fan-out: ref payloads reach the ref
// engine, every natural/sapi/legacy payload keeps hitting the platform
// SAPI engine, and a missing engine surfaces M95-001 semantics.
package tts
...
func TestRouterRoutesByEngine(t *testing.T) {
	...
	router := NewRouterEngineWithEngines(sapi, ref)
	...
	if len(sapi.calls) != 4 || len(ref.calls) != 1 {
		t.Fatalf("call fan-out wrong: sapi=%d ref=%d", len(sapi.calls), len(ref.calls))
	}
}
